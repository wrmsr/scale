...
public class TripsTimes
{
    ...
    private void calcGeoMean(int beg, int di, long[] insts, int which)
    {
        for (int i = beg; i <= di; i++) {
            ...
            for (int j = 0; j < numBenchmarks; j++) {
                ...
                switch (which) {
                    case 0:
                        min = benchmarks[j].minInsts;
                        max = benchmarks[j].maxInsts;
                        break;
                    case 1:
                        min = benchmarks[j].minBlocks;
                        max = benchmarks[j].maxBlocks;
                        break;
                    case 2:
                        min = benchmarks[j].minCycles;
                        max = benchmarks[j].maxCycles;
                        break;
                }
    ...
    private int nextToken(String line, int i)
    {
        ...
        int tokenb = i;
        while ((i < l) && (line.charAt(i) != ' ') && (line.charAt(i) != ',')) {
            i++;
        }
        int tokene = i;

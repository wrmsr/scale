...
public class X86Generator
        extends scale.backend.Generator
{
    ...
    {
        super(cg, new X86RegisterSet(), machine, features);

        assert (machine instanceof X86Machine) : "Not correct machine " + machine;

        this.un = new UniqueName("$$");

        // Determine what additional X86 features are available.

        X86Machine am = (X86Machine) machine;
    ...
    public void assemble(Emit emit, String source, Enumeration<String> comments)
    {
        ...
        X86Assembler asm = new X86Assembler(this, source, !nis);
    ...
    public int dataType(int size, boolean flt)
    {
        ...
        switch (size) {
            ...
            case 8:
                ...
                break;
            case 16:
                t = flt ? SpaceAllocation.DAT_LDBL : SpaceAllocation.DAT_LONG;
                break;
            default:

...
public class Opcodes
{
    ...
    public static String getOp(int opcode)
    {
        ...
        switch (fmt) {
            case 0:
                switch (ftn) {
                    case (ILLTRAP >> FTNSHFT) & 0xff:
                        return "illtrap";
                    case (BP >> FTNSHFT) & 0xff:
                        return "bp" + condi[cond];
                    case (B >> FTNSHFT) & 0xff:
                        return "b" + condi[cond];
                    case (BR >> FTNSHFT) & 0xff:
                        return "br" + rcond[cond];
                    case (SETHI >> FTNSHFT) & 0xff:
                        return "sethi";
                    case (FBP >> FTNSHFT) & 0xff:
                        return "fbp" + condf[cond];
                    case (FB >> FTNSHFT) & 0xff:
                        return "fb" + condf[cond];
                    case (OP070 >> FTNSHFT) & 0xff:
                        return "op070";
                }
                break;
            case 1:
            ...
            case 2:
                switch (ftn) {
                    case (ADD >> FTNSHFT) & 0xff:
                        return "add";
                    case (AND >> FTNSHFT) & 0xff:
                        return "and";
                    case (OR >> FTNSHFT) & 0xff:
                        return "or";
                    case (XOR >> FTNSHFT) & 0xff:
                        return "xor";
                    case (SUB >> FTNSHFT) & 0xff:
                        return "sub";
                    case (ANDN >> FTNSHFT) & 0xff:
                        return "andn";
                    case (ORN >> FTNSHFT) & 0xff:
                        return "orn";
                    case (XNOR >> FTNSHFT) & 0xff:
                        return "xnor";
                    case (ADDC >> FTNSHFT) & 0xff:
                        return "addc";
                    case (MULX >> FTNSHFT) & 0xff:
                        return "mulx";
                    case (UMUL >> FTNSHFT) & 0xff:
                        return "umul";
                    case (SMUL >> FTNSHFT) & 0xff:
                        return "smul";
                    case (SUBC >> FTNSHFT) & 0xff:
                        return "subc";
                    case (UDIVX >> FTNSHFT) & 0xff:
                        return "udivx";
                    case (UDIV >> FTNSHFT) & 0xff:
                        return "udiv";
                    case (SDIV >> FTNSHFT) & 0xff:
                        return "sdiv";
                    case (ADDCC >> FTNSHFT) & 0xff:
                        return "addcc";
                    case (ANDCC >> FTNSHFT) & 0xff:
                        return "andcc";
                    case (ORCC >> FTNSHFT) & 0xff:
                        return "orcc";
                    case (XORCC >> FTNSHFT) & 0xff:
                        return "xorcc";
                    case (SUBCC >> FTNSHFT) & 0xff:
                        return "subcc";
                    case (ANDNCC >> FTNSHFT) & 0xff:
                        return "andncc";
                    case (ORNCC >> FTNSHFT) & 0xff:
                        return "orncc";
                    case (XNORCC >> FTNSHFT) & 0xff:
                        return "xnorcc";
                    case (ADDCCC >> FTNSHFT) & 0xff:
                        return "andccc";
                    case (OP192 >> FTNSHFT) & 0xff:
                        return "op192";
                    case (UMULCC >> FTNSHFT) & 0xff:
                        return "umulcc";
                    case (SMULCC >> FTNSHFT) & 0xff:
                        return "smulcc";
                    case (SUBCCC >> FTNSHFT) & 0xff:
                        return "subccc";
                    case (OP1D2 >> FTNSHFT) & 0xff:
                        return "op1d2";
                    case (UDIVCC >> FTNSHFT) & 0xff:
                        return "udivcc";
                    case (SDIVCC >> FTNSHFT) & 0xff:
                        return "sdivcc";
                    case (TADDCC >> FTNSHFT) & 0xff:
                        return "taddcc";
                    case (TSUBCC >> FTNSHFT) & 0xff:
                        return "tsubcc";
                    case (TADDCCTV >> FTNSHFT) & 0xff:
                        return "taddcctv";
                    case (TSUBCCTV >> FTNSHFT) & 0xff:
                        return "tsubcctv";
                    case (MULSCC >> FTNSHFT) & 0xff:
                        return "mulscc";
                    case (SLL >> FTNSHFT) & 0xff:
                        return (subop == 0) ? "sll" : "sllx";
                    case (SRL >> FTNSHFT) & 0xff:
                        return (subop == 0) ? "srl" : "srlx";
                    case (SRA >> FTNSHFT) & 0xff:
                        return (subop == 0) ? "sra" : "srax";
                    case (RR >> FTNSHFT) & 0xff:
                        return (subop < 16) ? rr[subop] : "rdasr";
                    case (OP292 >> FTNSHFT) & 0xff:
                        return "op292";
                    case (RDPR >> FTNSHFT) & 0xff:
                        return "rdpr";
                    case (FLUSHW >> FTNSHFT) & 0xff:
                        return "flushw";
                    case (MOVCC >> FTNSHFT) & 0xff:
                        return "movcc";
                    case (SDIVX >> FTNSHFT) & 0xff:
                        return "sdivx";
                    case (POPC >> FTNSHFT) & 0xff:
                        return "popc";
                    case (MOVR >> FTNSHFT) & 0xff:
                        return "movr" + rcond[cond];
                    case (WR >> FTNSHFT) & 0xff:
                        return wr[subop];
                    case (SR >> FTNSHFT) & 0xff:
                        return (subop == 0) ? "saved" : "restored";
                    case (WRPR >> FTNSHFT) & 0xff:
                        return "wrpr";
                    case (OP332 >> FTNSHFT) & 0xff:
                        return "op332";
                    ...
                    case (FPop2 >> FTNSHFT) & 0xff:
                        ...
                        switch (row35) {
                            case 0X00:
                                switch (col35) {
                                    case 0X1:
                                        return "fmovs" + condf[cond];
                                    case 0X2:
                                        return "fmovd" + condf[cond];
                                    case 0X3:
                                        return "fmovq" + condf[cond];
                                }
                                break;
                            case 0X02:
                                switch (col35) {
                        ...
                        break;
                    case (IMPDEP1 >> FTNSHFT) & 0xff:
                        return "impdep1";
                    case (IMPDEP2 >> FTNSHFT) & 0xff:
                        return "impdep2";
                    case (JMPL >> FTNSHFT) & 0xff:
                        return "jmpl";
                    case (RETURN >> FTNSHFT) & 0xff:
                        return "return";
                    case (T >> FTNSHFT) & 0xff:
                        return "T" + condi[cond];
                    case (FLUSH >> FTNSHFT) & 0xff:
                        return "flush";
                    case (SAVE >> FTNSHFT) & 0xff:
                        return "save";
                    case (RESTORE >> FTNSHFT) & 0xff:
                        return "restore";
                    case (DONE >> FTNSHFT) & 0xff:
                        return (subop == 0) ? "done" : "retry";
                    case (OP3F2 >> FTNSHFT) & 0xff:
                        return "op3f2";
                }
                break;
            case 3:
                switch (ftn) {
                    ...
                    case (STD >> FTNSHFT) & 0xff:
                        return "std";
                    case (LDSW >> FTNSHFT) & 0xff:
                        return "ld";
                    case (LDSB >> FTNSHFT) & 0xff:
                        return "ldsb";
                    case (LDSH >> FTNSHFT) & 0xff:
                        return "ldsh";
                    case (LDX >> FTNSHFT) & 0xff:
                        return "ldx";
                    case (OP0C3 >> FTNSHFT) & 0xff:
                        return "op0c3";
                    case (LDSTUB >> FTNSHFT) & 0xff:
                        return "ldstub";
                    case (STX >> FTNSHFT) & 0xff:
                        return "stx";
                    case (SWAP >> FTNSHFT) & 0xff:
                        return "swap";
                    ...
                    case (LDSHA >> FTNSHFT) & 0xff:
                        return "ldsha";
                    case (LDXA >> FTNSHFT) & 0xff:
                        return "ldxa";
                    case (OP1C3 >> FTNSHFT) & 0xff:
                        return "op1c3";
                    case (LDSTUBA >> FTNSHFT) & 0xff:
                        return "ldstuba";
                    case (STXA >> FTNSHFT) & 0xff:
                        return "stxa";
                    case (SWAPA >> FTNSHFT) & 0xff:
                        return "swapa";
                    ...
                    case (STDF >> FTNSHFT) & 0xff:
                        return "std";
                    case (OP283 >> FTNSHFT) & 0xff:
                        return "op283";
                    case (OP293 >> FTNSHFT) & 0xff:
                        return "op293";
                    case (OP2A3 >> FTNSHFT) & 0xff:
                        return "op2a3";
                    case (OP2B3 >> FTNSHFT) & 0xff:
                        return "op2b3";
                    case (OP2C3 >> FTNSHFT) & 0xff:
                        return "op2c3";
                    case (PREFETCH >> FTNSHFT) & 0xff:
                        return "prefetch";
                    case (OP2E3 >> FTNSHFT) & 0xff:
                        return "op2e3";
                    case (OP2F3 >> FTNSHFT) & 0xff:
                        return "op2f3";
                    case (LDFA >> FTNSHFT) & 0xff:
                        return "ldfa";
                    case (OP313 >> FTNSHFT) & 0xff:
                        return "op313";
                    ...
                    case (STFA >> FTNSHFT) & 0xff:
                        return "stfa";
                    case (OP353 >> FTNSHFT) & 0xff:
                        return "op353";
                    case (STQFA >> FTNSHFT) & 0xff:
                        return "stqfa";
                    case (STDFA >> FTNSHFT) & 0xff:
                        return "stdfa";
                    case (OP383 >> FTNSHFT) & 0xff:
                        return "op383";
                    case (OP393 >> FTNSHFT) & 0xff:
                        return "op393";
                    case (OP3A3 >> FTNSHFT) & 0xff:
                        return "op3a3";
                    case (OP3B3 >> FTNSHFT) & 0xff:
                        return "op3b3";
                    case (CASA >> FTNSHFT) & 0xff:
                        return "case";
                    case (PREFETCHA >> FTNSHFT) & 0xff:
                        return "prefetcha";
                    case (CASXA >> FTNSHFT) & 0xff:
                        return "casxa";
                    case (OP3F3 >> FTNSHFT) & 0xff:
                        return "op3f3";
                }
                break;
            case 15:
                return "nop\t! 0x" + Integer.toHexString(opcode) + " ";
        }

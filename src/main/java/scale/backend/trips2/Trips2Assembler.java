...
public final class Trips2Assembler
        extends scale.backend.Assembler
{
    ...
    public void assembleDataAreaHeader(Emit emit, int kind)
    {
        ...
        switch (kind) {
            case Trips2Generator.BSS:
                emit.emit("; BSS ");
                break;
            case Trips2Generator.SBSS:
                emit.emit("SBSS ");
                break;
            case Trips2Generator.DATA:
                emit.emit("\t.data\t");
                break;
            case Trips2Generator.LIT4:
                emit.emit("LIT4 ");
                break;
            case Trips2Generator.LIT8:
                emit.emit("LIT8 ");
                break;
            case Trips2Generator.LITA:
                emit.emit("LITA ");
                break;
            case Trips2Generator.RCONST:
                emit.emit("RCONST ");
                break;
            case Trips2Generator.RDATA:
                emit.emit("\t.rdata\t");
                break;
            case Trips2Generator.SDATA:
                emit.emit("SDATA ");
                break;
            case Trips2Generator.TEXT:
                emit.emit("\t.text\t");
                break;
            default:
        ...
        for (int i = 0; i < vl; i++) {
            ...
            switch (loc) {
                case IN_REGISTER:
                    ...
                    break;
                case ON_STACK:
                    emit.emit("(stack " + vd.getDisplacement() + ")");
                    break;
                case IN_COMMON:
                    emit.emit("common");
                    break;

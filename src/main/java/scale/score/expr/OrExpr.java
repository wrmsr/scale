package scale.score.expr;

import scale.clef.expr.Literal;
import scale.clef.type.Type;
import scale.common.HashMap;
import scale.score.Predicate;

/**
 * This class represents the boolean or operation.
 * <p/>
 * $Id: OrExpr.java,v 1.25 2007-04-27 18:04:37 burrill Exp $
 * <p/>
 * Copyright 2008 by the
 * <a href="http://ali-www.cs.umass.edu/">Scale Compiler Group</a>,<br>
 * <a href="http://www.cs.umass.edu/">Department of Computer Science</a><br>
 * <a href="http://www.umass.edu/">University of Massachusetts</a>,<br>
 * Amherst MA. 01003, USA<br>
 * All Rights Reserved.<br>
 */
public class OrExpr
        extends BinaryExpr
{
    public OrExpr(Type t, Expr e1, Expr e2)
    {
        super(t, e1, e2);
    }

    public Expr copy()
    {
        return new OrExpr(getType(), getLeftArg().copy(), getRightArg().copy());
    }

    public void visit(Predicate p)
    {
        p.visitOrExpr(this);
    }

    public String getDisplayLabel()
    {
        return "||";
    }

    /**
     * Return the constant value of the expression.
     * Follow use-def links.
     *
     * @see scale.common.Lattice
     */
    public Literal getConstantValue(HashMap<Expr, Literal> cvMap)
    {
        Literal r = cvMap.get(this);
        if (r != null) {
            return r;
        }

        Literal la = getLeftArg().getConstantValue(cvMap);
        Literal ra = getRightArg().getConstantValue(cvMap);
        r = scale.common.Lattice.orCond(getCoreType(), la, ra);

        cvMap.put(this, r);
        return r;
    }

    /**
     * Return the constant value of the expression.
     * Do not follow use-def links.
     *
     * @see scale.common.Lattice
     */
    public Literal getConstantValue()
    {
        Literal la = getLeftArg().getConstantValue();
        Literal ra = getRightArg().getConstantValue();

        return scale.common.Lattice.orCond(getCoreType(), la, ra);
    }

    public boolean isLeftDistributive()
    {
        if (getLeftArg() instanceof AndExpr) {
            return true;
        }
        else {
            return false;
        }
    }

    public boolean isRightDistributive()
    {
        if (getRightArg() instanceof AndExpr) {
            return true;
        }
        else {
            return false;
        }
    }
}

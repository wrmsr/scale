 ...
 * or <a href="http://www.cs.umd.edu/projects/omega">
 * http://www.cs.umd.edu/projects/omega</a>
 * </blockquote>
 */
        ...
        extends Formula
{
    ...
    public RelBody(RelBody r)
    {
        ...
        this.ref_count = 0;
        this.inNames = copyCNames(r.inNames);
        this.outNames = copyCNames(r.outNames);
    ...
    public RelBody(RelBody r, Conjunct c)
    {
        ...
        simplifiedDNF.addConjunct(c.copy(this, this));
    ...
    public Vector<VarDecl> copyVars(Vector<VarDecl> vars)
    {
        ...
        Vector<VarDecl> nv = new Vector<VarDecl>(vars.size());
        for (VarDecl vd : vars) {
            nv.add(vd);
        }
    ...
    public Vector<CName> copyCNames(Vector<CName> vars)
    {
        ...
        Vector<CName> nv = new Vector<CName>(vars.size());
        for (CName vd : vars) {
            nv.add(vd);
        }
    ...
    private void dumpInNames(Vector<CName> names, int n)
    {
        System.out.println("Dump InNames");
        for (int i = 1; i <= n; i++) {
            CName cs = names.elementAt(i);
            String na = cs.name();
            if (na.equals("")) {
    ...
    {
        throw new scale.common.InternalError("RelBody.copy");
    }
    ...
    private int maxUfsArity()
    {
        ...
        for (int i = 0; i < gvl; i++) {
            VarDecl v = symbolic.elementAt(i);
    ...
    private int maxSharedUfsArity()
    {
        ...
        for (int i = 0; i < gvl; i++) {
            VarDecl v = symbolic.elementAt(i);
            for (int j = 0; j < gvl; j++) {
                VarDecl v2 = symbolic.elementAt(j);
    ...
    public VarDecl outputVar(int nth)
    {
        ...
        CName c = outNames.elementAt(nth);
    ...
    public VarDecl getLocal(VarDecl v)
    {
        ...
        throw new scale.common.InternalError("Can only get local for variable with global scope");
    }
    ...
    public void nameInputVar(int nth, String s)
    {
        if ((nth < 1) || (nth > number_input) || (isSet() && (omegaLib.skipSetChecks <= 0))) {
            throw new scale.common.InternalError("Invalid input variable " + nth);
        }
    ...
    public void nameOutputVar(int nth, String s)
    {
        if ((nth < 1) || (nth > number_output) || (isSet() && (omegaLib.skipSetChecks <= 0))) {
            throw new scale.common.InternalError("Invalid output variable " + nth);
        }
    ...
    {
        if (name == null) {
            throw new scale.common.InternalError("addInputVar");
        }
    ...
    {
        if (name == null) {
            throw new scale.common.InternalError("addSetVar");
        }
    ...
    {
        if (name == null) {
            throw new scale.common.InternalError("addOutputVar");
        }
...
//      v.printVarAddrs(s);
    ...
    public String printWithSubsToString(boolean printSym, boolean newline)
    {
        ...
        Vector<Conjunct> cl = R.queryDNF().getConjList();
        int cll = cl.size();
        for (int icl = 0; icl < cll; icl++) {
            Conjunct c = cl.elementAt(icl);
    ...
    /* Do actual printing of Conjunct C as a relation */
        ...
        if (printSym) {
            if (symbolic.size() != 0) {
                ...
                for (int j = 0; j < sl; j++) {
                    VarDecl v = symbolic.elementAt(j);
                    s.append(v.name(omegaLib));
                    if (j < sl - 1) {
                        s.append(",");
                    }
...
//      System.out.print(")");
    ...
    public boolean isSatisfiable()
    {
        ...
        olAssert(ubs == isLowerBoundSatisfiable());
        return ubs;
    }
    ...
    public RelBody extractDNFByCarriedLevel(int level, int direction)
    {
        ...
        extracted.symbolic = copyVars(r.symbolic);
        ...
        for (Conjunct curr = r.simplifiedDNF.removeFirstConjunct();
            ...
            if (!curr.checkLeading0s(level)) {
                throw new scale.common.InternalError("leading 0s");
            }
    ...
    public void setupNames()
    {
        ...
        for (int i = 1; i <= numberInput(); i++) {
            omegaLib.newInputVar(i).nameVariable(inNames.elementAt(i).name());
        }

        for (int i = 1; i <= numberOutput(); i++) {
            omegaLib.newOutputVar(i).nameVariable(outNames.elementAt(i).name());
        }

        HashSet<GlobalVarDecl> gbls = new HashSet<GlobalVarDecl>(11);

        omegaLib.wildCardInstanceNumber = 0;

        for (int i = 0; i < symbolic.size(); i++) {
            VarDecl s = symbolic.elementAt(i);
            gbls.add(s.getGlobalVar());
        }

        Iterator<GlobalVarDecl> eg = gbls.iterator();
        while (eg.hasNext()) {
            GlobalVarDecl g = eg.next();
        ...
        Iterator<GlobalVarDecl> it = gbls.iterator();
        while (it.hasNext()) {
            GlobalVarDecl g = it.next();
    ...
    public void beautify()
    {
        if (numberOfChildren() != 1) {
            throw new scale.common.InternalError("Length of children");
        }
        ...
        if (numberOfChildren() != 1) {
            throw new scale.common.InternalError("Length of children 2");
        }
    }
    ...
    public void reverseLeadingDirInfo()
    {
        ...
        if ((simplifiedDNF != null) || (numberOfChildren() != 1)) {
            throw new scale.common.InternalError("Num children = " + numberOfChildren());
        }
    ...
    {
        if (simplifiedDNF == null) {
            simplifiedDNF = children().remove(0).DNFize();
    ...
    public Conjunct findAvailableConjunct()
    {
        ...
        Formula kid = children().elementAt(0);  // RelBodies have only one child
    ...
    public boolean align(RelBody newr, FExists fe, Formula f, Mapping mapping)
    {
        ...
        for (int i = 1; i <= numberInput(); i++) {
            ...
            String this_name = inNames.elementAt(i).name();
            ...
            switch (mapping.getMapInKind(i)) {
                ...
                case VarDecl.SET_VAR:
                    ...
                    olAssert((newrIsSet &&
                            mapping.getMapInKind(i) == VarDecl.SET_VAR) ||
                            ((!newrIsSet && mapping.getMapInKind(i) == VarDecl.INPUT_VAR)));
                    ...
                    String new_name = newr.inNames.elementAt(new_pos).name();
                ...
                case VarDecl.OUTPUT_VAR:
                    ...
                    String new_nameo = newr.outNames.elementAt(new_poso).name();
                ...
                case VarDecl.EXISTS_VAR:
                    ...
                    break;
                default:
                    throw new scale.common.InternalError("Unsupported var type in align");
            }
        }
        ...
        for (int i = 1; i <= number_output; i++) {
            ...
            String this_name = outNames.elementAt(i).name();
            ...
            switch (mapping.getMapOutKind(i)) {
                ...
                case VarDecl.SET_VAR:
                    ...
                    olAssert((newrIsSet &&
                            (mapping.getMapOutKind(i) == VarDecl.SET_VAR)) ||
                            ((!newrIsSet && (mapping.getMapOutKind(i) == VarDecl.INPUT_VAR))));
        ...
        Vector<VarDecl> oldSym = symbolic;
        int l = oldSym.size();
        for (int i = 1; i <= l; i++) {
            VarDecl v = oldSym.elementAt(i - 1);
            ...
            if (gva > 0) {
                ...
                if (new_of == VarDecl.UNKNOWN_TUPLE) {
                    ...
                    if ((f.nodeType() == Formula.OP_CONJUNCT) && (f.reallyConjunct().variables().indexOf(v) != 0)) {
                        throw new scale.common.InternalError("v unused");
                    }
        ...
        if (sym_remapped || input_remapped || output_remapped) {
            ...
            HashSet<Object> vt = new HashSet<Object>(11);
            boolean combine = false;
            for (int i = 1; !combine && i <= numberInput(); i++) {
                VarDecl t = inputVar(i);
        ...
        if (fe != null) {
            ...
            for (int i = 0; i < fel; i++) {
                VarDecl v = fe.myLocals.elementAt(i);
    ...
    private boolean gistSingleConjunctSub(RelBody r2, int effort)
    {
        ...
        if (!r2.isUpperBoundSatisfiable()) {
            throw new scale.common.InternalError("Gist: second operand is FALSE");
        }
        ...
        if (known == null) {
            throw new scale.common.InternalError("Gist: second operand has more than 1 conjunct");
        }
    ...
    private RelBody MapRel1(Mapping map,
            boolean isAndNot,
            int number_input,
            int number_output,
            boolean invalidate_resulting_leading_info,
            boolean finalize)
    {
        map.getRelationArityFromOneMapping();

        RelBody outputRelBody = new RelBody(omegaLib,
                (number_input == -1) ? map.getInReq() : number_input,
                (number_output == -1) ? map.getOutReq() : number_output);
    ...
    public RelBody after(int carried_by, int new_output, int dir)
    {
        ...
        h.updateCoefficient(omegaLib.getVarIDS(1), -dir);
    ...
    public RelBody deltasToRelation(int n_inputs, int n_outputs)
    {
        ...
        if (r.maxUfsArity() > 0) {
            throw new scale.common.InternalError("\"Deltas\" not ready for UFS yet"); // FERD
        }
    ...
    private RelBody MapAndCombineRel2(RelBody r2,
            Mapping mapping1,
            Mapping mapping2,
            boolean isOr,
            boolean comp,
            ...
            int number_output)
    {
        ...
        RelBody r3 = new RelBody(omegaLib,
                (number_input == -1) ? mapping1.getInReq() : number_input,
                (number_output == -1) ? mapping1.getOutReq() : number_output);
    ...
    public RelBody union(RelBody r2)
    {
        ...
        if ((in != r2.number_input) || (out != r2.number_output)) {
            throw new scale.common.InternalError("in & out");
        }
    ...
    public RelBody restrictDomain(RelBody r2)
    {
        ...
        if (!r2.isSet() || (in != r2.numberSet())) {
            throw new scale.common.InternalError("set");
        }
    ...
    public RelBody composition(RelBody r2)
    {
        ...
        if (r2.isSet()) {
            ...
            if (r2.numberSet() != r1.number_input) {
                ...
                throw new scale.common.InternalError("composition");
            }
            ...
            throw new scale.common.InternalError("Can't compose RelBody and set with function symbols");
        }
        ...
        if (r2.number_output != r1.number_input) {
            ...
            throw new scale.common.InternalError("Composition2");
        }
        ...
        Vector<VarDecl> middle_tuple = exists.declareTuple(mid_size);
        HashMap<GlobalVarDecl, VarDecl> lost_functions = new HashMap<GlobalVarDecl, VarDecl>(11); // Map from global variable to variable.
        FOr result_conjs = exists.addOr();
        Vector<Conjunct> cl1 = d1.getConjList();
        int cll1 = cl1.size();

        for (int icl1 = 0; icl1 < cll1; icl1++) {
            Conjunct conj1 = cl1.elementAt(icl1);
            Vector<Conjunct> cl2 = d2.getConjList();
            int cll2 = cl2.size();

            for (int icl2 = 0; icl2 < cll2; icl2++) {
                Conjunct conj2 = cl2.elementAt(icl2);
...
//      conj1.studyEvacuation(conj2, a);
                ...
                Vector<VarDecl> remapped = new Vector<VarDecl>();
                ...
                Vector<VarDecl> vars2 = copy2.variables();
                int l2 = vars2.size();
                for (int vi = 0; vi < l2; vi++) {
                    VarDecl func = vars2.elementAt(vi);
                    ...
                    if ((f.arity() > 0) && (func.functionOf() == VarDecl.OUTPUT_TUPLE)) {
                        if (c2L0 >= f.arity()) {
                            ...
                            VarDecl f_quantified = lost_functions.get(f);
                ...
                for (int i = 1; i <= mid_size; i++) {
                    r2.outputVar(i).resetRemapField(middle_tuple.elementAt(i));
                }
                ...
                Vector<VarDecl> vars1 = copy1.variables();
                int l1 = vars1.size();
                for (int vi = 0; vi < l1; vi++) {
                    VarDecl func = vars1.elementAt(vi);
                    if (func.kind() != VarDecl.GLOBAL_VAR) {
                        continue;
                    }
                ...
                for (int i = 1; i <= mid_size; i++) {
                    r1.inputVar(i).resetRemapField(middle_tuple.elementAt(i));
                }
                ...
                Vector<VarDecl> vars = conj3.variables();
                int l3 = vars.size();
                for (int vi = 0; vi < l3; vi++) {
                    VarDecl func = vars.elementAt(vi);
                    if (func.kind() != VarDecl.GLOBAL_VAR) {
                        continue;
                    }
    ...
    public boolean doSubsetCheck(RelBody r2)
    {
        ...
        Vector<Conjunct> cl1 = r1.queryDNF().getConjList();
        int cll1 = cl1.size();
        for (int icl1 = 0; icl1 < cll1; icl1++) {
            Conjunct pd = cl1.elementAt(icl1);
    ...
    public RelBody project(GlobalVarDecl g)
    {
        ...
        if (g.arity() == 0) {
            if (!r.hasLocal(g)) {
                throw new scale.common.InternalError("Project: RelBody doesn't contain variable to be projected");
            }
            ...
            if (hi) {
                ...
                if (!r.removeGlobalDecl(v)) {
                    throw new scale.common.InternalError("Project: Variable to be projected doesn't exist");
                }
    ...
    public RelBody projectSym()
    {
        ...
        FExists ex = r.addExists();
        Vector<VarDecl> gbls = r.getGlobalDecls();
        int l = gbls.size();
        for (int i = 0; i < l; i++) {
            VarDecl v = gbls.elementAt(i);
            v.resetRemapField(ex.declare(v.baseName()));
        }
    ...
    public RelBody project(int pos, int vkind)
    {
        ...
        switch (vkind) {
            ...
            case VarDecl.SET_VAR:
                ...
                break;
            default:
                throw new scale.common.InternalError("Project variable kind");
        }
    ...
    private void printRs(Vector<RelBody> rs)
    {
        System.out.println("Rs:");
        int rl = rs.size();
        for (int i = 0; i < rl; i++) {
            RelBody r = rs.elementAt(i);
            System.out.println("#");
            System.out.print(i);
            System.out.print(" : ");
    ...
    private void handleVariable(Conjunct conj,
            FAnd and_node,
            HashMap<Equation, VarDecl> gMap,
            HashMap<Equation, VarDecl> eMap,
            VarDecl v)
    {
        omegaLib.useUglyNames++;
        if (omegaLib.trace) {
    ...
    private RelBody farkas(int op)
    {
        ...
        Vector<VarDecl> vars = copyVars(R.getGlobalDecls());
        if (R.isSet()) {
        ...
        Vector<VarDecl> owners = new Vector<VarDecl>();

        //    Map<VarDecl, VarDecl_Set> connectedVariables(empty);

        HashMap<VarDecl, HashSet<VarDecl>> connectedVariables = new HashMap<VarDecl, HashSet<VarDecl>>(11);

        if (op == Decoupled_Farkas) {
            Vector<VarDecl> gl = R.getGlobalDecls();
            int gll = gl.size();
            for (int gli = 0; gli < gll; gli++) {
                VarDecl v = gl.elementAt(gli);
                if (v.kind() != VarDecl.GLOBAL_VAR) {
                    continue;
                }
            ...
            int vvl = vars.size();
            for (int i = 0; i < vvl; i++) {
                VarDecl v3 = vars.elementAt(i);
                VarDecl owner = v3.UFOwner();
                HashSet<VarDecl> hs = connectedVariables.get(owner);
                if (hs == null) {
                    hs = new HashSet<VarDecl>(3);
                    connectedVariables.put(owner, hs);
                }
                hs.add(v3);
            }

            Enumeration<VarDecl> ek = connectedVariables.keys();
            while (ek.hasMoreElements()) {
                VarDecl v = ek.nextElement();
                HashSet<VarDecl> hs = connectedVariables.get(v);
                owners.addElement(v);
                if (omegaLib.trace) {
                    System.out.print(v.displayName(omegaLib));
                    System.out.print(":");
                    Iterator<VarDecl> it = hs.iterator();
                    while (it.hasNext()) {
                        VarDecl v2 = it.next();
                        System.out.print(" ");
        ...
        while (((op == Decoupled_Farkas) && (oli < ol)) || ((op != Decoupled_Farkas) && firstGroup)) {
            VarDecl owner = owners.elementAt(oli);
            ...
            for (int cli = 0; cli < cll; cli++) {
                ...
                HashMap<Equation, VarDecl> gMap = new HashMap<Equation, VarDecl>(11);
                HashMap<Equation, VarDecl> eMap = new HashMap<Equation, VarDecl>(11);
                ...
                for (int j = 0; j < neqs; j++) {
                    ...
                    if (op == Decoupled_Farkas) {
                        ...
                        Vector<VarDecl> vl = s.variables();
                        int vll = vl.size();
                        for (int vli = 0; vli < vll; vli++) {
                            VarDecl v = vl.elementAt(vli);
    ...
    private RelBody fastTightHull(RelBody input_H)
    {
        ...
        HashSet<VarDecl> vars = R.queryDNF().fastTightHull();
    ...
    private RelBody quickHull(Vector<RelBody> Rs)
    {
        olAssert(0 != Rs.size());

        RelBody R1 = Rs.elementAt(0);
        if (Rs.size() == 1) {
            return R1;
        }
    ...
    private RelBody betterHull(RelBody knownHull, Vector<RelBody> Rs, boolean stridesAllowed, boolean checkSubsets)
    {
    ...
    private void genRelationsFromConjuncts(Vector<RelBody> rs)
    {
        Vector<Conjunct> cl = queryDNF().getConjList();
        int cll = cl.size();
        for (int cli = 0; cli < cll; cli++) {
            Conjunct c = cl.elementAt(cli);
    ...
    public RelBody hull(boolean stridesAllowed, int effort, RelBody knownHull)
    {
        ...
        Vector<RelBody> rs = new Vector<RelBody>();
    ...
    private RelBody hull(Vector<RelBody> rs,
            boolean[] validMask,
            int effort,
            ...
            RelBody knownHull)
    {
        ...
        Vector<RelBody> rs2 = new Vector<RelBody>();
        int rl = rs.size();
        for (int i = 0; i < rl; i++) {
            if (!validMask[i]) {
                continue;
    ...
    private RelBody vennDiagramForm(Vector<RelBody> Rs, int next, boolean anyPositives, int weight)
    {
    ...
    private RelBody vennDiagramForm(RelBody context, Vector<RelBody> Rs)
    {
        if (context == null) {
    ...
    public RelBody gistSingleConjunct(RelBody R2, int effort)
    {
        ...
        olAssert(((R1.number_input == R2.number_input) &&
                (R1.number_output == R2.number_output)) ||
                ((R2.number_input == 0) &&
                        (R2.number_output == 0)));
        R1.simplify();
        R2.simplify();
    ...
    public RelBody gist(RelBody R2, int effort)
    {
        ...
        Vector<Conjunct> cl2 = R2.simplifiedDNF.getConjList();
        int cll2 = cl2.size();
        Vector<Conjunct> cl1 = R1.simplifiedDNF.getConjList();
        int cll1 = cl1.size();

        for (int icl2 = 0; icl2 < cll2; icl2++) {
            Conjunct c2 = cl2.elementAt(icl2);
            RelBody G2 = R1.falseRelation();

            for (int icl1 = 0; icl1 < cll1; icl1++) {
                Conjunct c1 = cl1.elementAt(icl1);
    ...
    /* add all stride constrains if they do exist */
        ...
        if (c.locals().size() > 0) { // there are local variables
            ...
            Equation[] eqs = c.getEQs();
            int neqs = c.getNumEQs();
            Vector<VarDecl> vars = c.variables();
            int l = vars.size();
    ...
    */
    ...
    private void appendClausesToList(Vector<Relation> l)
    {
        makeLevelCarriedTo(number_input);
        simplify(2, 2);
    ...
    private void printRelationList(Vector<Relation> l)
    {
        int ll = l.size();
        for (int i = 0; i < ll; i++) {
            RelBody r = l.elementAt(i).getRelBody();
            r.printWithSubs();
        }
    }
    ...
    public RelBody transitiveClosure0(int maxExpansion, RelBody iterationSpace)
    {
        ...
        if (r.maxUfsArity() > 0) {
            throw new scale.common.InternalError("Can't take transitive closure with UFS yet.");
        }
        ...
        RelBody result = r.falseRelation();
        Vector<Relation> firstChoice = new Vector<Relation>();
        Vector<Relation> secondChoice = new Vector<Relation>();
        ...
        for (int i = 0; i < sll; i++) {
            RelBody sli = firstChoice.elementAt(i).getRelBody();
            if (first_conj) {
                first_conj = false;
            }
            else {
                ...
                if (C_plus != null) {
                    firstChoice.setElementAt(new Relation(omegaLib, C_plus), i);
                }
            }
        }
        ...
        while ((0 != firstChoice.size()) || (0 != secondChoice.size())) {
            ...
            RelBody R;
            if (0 != firstChoice.size()) {
                R = firstChoice.remove(0).getRelBody();
            }
            else {
                R = secondChoice.remove(0).getRelBody();
            }
            ...
            if ((rPlus == null) && (numFails < firstChoice.size())) {
                numFails++;
                firstChoice.addElement(new Relation(omegaLib, R));
                if (omegaLib.trace) {
            ...
            if (!known && (expansion == 0)) {
                ...
                Vector<Relation> list1 = firstChoice;
                Vector<Relation> list2 = secondChoice;
                int sl = list1.size();
                for (int sli = 0; true; sli++) {
                    ...
                    RelBody C = list1.elementAt(sli).getRelBody();
                    ...
                    if (C.equal(Rz_C_Rz)) {
                        ...
                        list1.setElementAt(new Relation(omegaLib, s), sli);
    ...
    public RelBody transitiveClosure(int maxExpansion, RelBody iterationSpace)
    {
        ...
        Vector<Conjunct> cl = conditions.queryDNF().getConjList();
        int cll = cl.size();
        for (int i = 0; i < cll; i++) {
            Conjunct c = cl.elementAt(i);

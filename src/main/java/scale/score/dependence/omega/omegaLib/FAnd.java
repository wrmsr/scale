        ...
        extends Formula
{
    ...
    public Conjunct findAvailableConjunct()
    {
        int l = myChildren.size();
        for (int i = 0; i < l; i++) {
            Formula child = myChildren.elementAt(i);
    ...
    public void beautify()
    {
        ...
        Stack<Formula> uglies = new Stack<Formula>();
        Stack<Formula> beauties = new Stack<Formula>();

        Vector<Formula> ch = children();
        while (true) {
            int l = ch.size();
            if (l == 0) {
                break;
    ...
    public DNF DNFize()
    {
        ...
        Vector<DNF> pos_dnfs = new Vector<DNF>();
        ...
        if (myChildren.size() == 0) {
            ...
            while (myChildren.size() != 0) {
                Formula carg = myChildren.remove(0);
                if (carg.nodeType() == OP_NOT) {
                    // DNF1 & ~DNF2 . DNF
                    DNF dnf = carg.myChildren.remove(0).DNFize();
            ...
            if (!JustReturnDNF) {
                ...
                if ((positive_conjunct == null) && (pos_dnfs.size() == 1)) {
                    if (omegaLib.trace) {
                        ...
                        pos_dnfs.elementAt(1).prefixPrint();
                    ...
                    new_dnf.join_conjAndNotDnf(pos_dnfs.elementAt(1), neg_conjs);
                }
                else if ((positive_conjunct == null) && neg_conjs.isDefinitelyFalse()) {
                    new_dnf = pos_dnfs.elementAt(0);
                    pos_dnfs.setElementAt(null, 0);
                    int pll = pos_dnfs.size();
                    for (int j = 1; j < pll; j++) {
                        DNF pos_dnf = pos_dnfs.elementAt(j);

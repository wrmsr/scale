...
public abstract class Formula
{
    ...
    protected Formula(OmegaLib omegaLib, Formula p, RelBody r)
    {
        ...
        this.myParent = p;
        this.myRelation = r;
        this.myChildren = new Vector<Formula>();
    }
    ...
    public void delete()
    {
        ...
        for (int j = 0; j < l; j++) {
            Formula c = myChildren.elementAt(j);
            c.delete();
        }
        myChildren = null;
    }
    ...
    public void setFinalized()
    {
        ...
        Formula f = myChildren.elementAt(0);
        f.setFinalized();
    }
    ...
    protected void olAssert(boolean t)
    {
        ...
        throw new scale.common.InternalError(this.getClass().getName());
    }
    ...
    protected void olAssert(boolean t, String msg)
    {
        ...
        throw new scale.common.InternalError(this.getClass().getName() + ": " + msg);
    }
    ...
    public FForall addForall()
    {
        assertNotFinalized();
        if (!canAddChild()) {
            throw new scale.common.InternalError("acan't add child!");
        }
    ...
    {
        return myChildren.remove(0);
    }

    protected Formula getFirstChild()
    {
        return myChildren.elementAt(0);
    }

    protected void removeChild(Formula kid)
    {
        if (kid.myParent != this) {
            throw new scale.common.InternalError("Incorrect Formula structure.");
        }
        ...
        throw new scale.common.InternalError("Child to be removed not found in child list");
    }
    ...
    {
        if (!canAddChild()) {
            throw new scale.common.InternalError("Can't add child to " + this);
        }
    ...
    {
        throw new scale.common.InternalError("pushExists");
    }
    ...
    public void verifytree() // should be const
    {
        ...
        for (int i = 0; i < l; i++) {
            ...
            if (c.myParent != this) {
                throw new scale.common.InternalError("Child not child of parent.");
            }
            if (c.myRelation != this.myRelation) {
                throw new scale.common.InternalError("Child relation differs.");
            }
            c.verifytree();
        }
    }
    ...
    public void assertNotFinalized()
    {
        ...
        if (relation().isFinalized()) {
            throw new scale.common.InternalError("Relation is finalized.");
        }
        if (relation().isShared()) {
            throw new scale.common.InternalError("Relation is shared.");
        }
    }

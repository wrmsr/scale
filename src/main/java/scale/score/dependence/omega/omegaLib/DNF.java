...
public final class DNF
{
    ...
    private Vector<Conjunct> conjList; // List of Conjunct.
    public int id;

    public DNF(OmegaLib omegaLib)
    {
        this.omegaLib = omegaLib;
        this.conjList = new Vector<Conjunct>();
    ...
    public DNF copy(OmegaLib omegaLib, RelBody rel_body)
    {
        ...
        int l = conjList.size();
        for (int i = 0; i < l; i++) {
            Conjunct conj = conjList.elementAt(i);
            new_dnf.addConjunct(conj.copy(rel_body, rel_body));
        }
        return new_dnf;
    }
    ...
    public void print()
    {
        ...
        for (int i = 0; i < l; i++) {
            Conjunct p = conjList.elementAt(i);
            if (live) {
    ...
    public String printToString()
    {
        ...
        for (int i = 0; i < l; i++) {
            Conjunct c = conjList.elementAt(i);
            if (i > 0) {
                s.append(" && ");
            }
    ...
    public void prefixPrint(boolean debug, boolean parent_names_setup)
    {
        ...
        Vector<VarDecl> all_vars = new Vector<VarDecl>();
        if ((0 == omegaLib.useUglyNames) && !parent_names_setup) {
            ...
            for (int k = 0; k < al; k++) {
                VarDecl v = all_vars.elementAt(k);
                if (v.hasName()) {
        ...
        for (int j = 0; j < l; j++) {
            Conjunct conj = conjList.elementAt(j);
            System.out.print("#");
        ...
        int al = all_vars.size();
        for (int k = 0; k < al; k++) {
            omegaLib.decrement(all_vars.elementAt(k));
        }
    ...
    public int queryLeadingDir()
    {
        ...
        for (int i = 0; i < l; i++) {
            ...
            else if ((glz != rlz) || (result != conj.queryLeadingDir())) {
                return 0;
            }
        }
    ...
    public void makeLevelCarriedTo(int level)
    {
        ...
        if (length() > 0) {
            RelBody bodys = (conjList.elementAt(0)).relation();
    ...
    {
        conjList.addVectors(dnf.conjList);
        if (conjList.size() > maxSize) {
            maxSize = conjList.size();
        }
        dnf.delete();
    ...
    public void removeConjunct(Conjunct c)
    {
        ...
        throw new scale.common.InternalError("DNF.removeConjunct: no such conjunct " + c);
    }
    ...
    public DNF DNFandDNF(DNF dnf2)
    {
        ...
        for (int i = 0; i < l; i++) {
            Conjunct p = dnf2.conjList.elementAt(i);
    ...
    /* Compute gists of negative conjuncts given positive conjunct */
        ...
        while (c0_updated) {
            ...
            int l = conjList.size();
            for (int i = 0; i < l; i++) {
                Conjunct neg_conj = conjList.elementAt(i);
    ...
    /* Negate minimal conjunct, AND result with positive conjunct */
        ...
        if (live_count > 1) {
            Vector<Conjunct> cl = new_pos.conjList;
            int cll = cl.size();
            for (int i = 0; i < cll; i++) {
                Conjunct conj = cl.elementAt(i);
    ...
    public void dimensions(RelBody r, int[] dims)
    {
        ...
        for (int i = 0; i < cll; i++) {
            Conjunct s = conjList.elementAt(i);
    ...
    public boolean redSimplifyProblem(DNF dnf2)
    {
        ...
        for (int i1 = 0; i1 < cll1; i1++) {
            Conjunct c1 = conjList.elementAt(i1);
            int cll2 = dnf2.conjList.size();
            boolean found = false;
            for (int i2 = 0; i2 < cll2; i2++) {
                Conjunct c2 = dnf2.conjList.elementAt(i2);
                if (!c2.isExact()) {
                    continue;
                }
    ...
    {
        HashSet<VarDecl> vars = new HashSet<VarDecl>(11);
        int cll = conjList.size();
        for (int i = 0; i < cll; i++) {
            Conjunct c = conjList.elementAt(i);
    ...
    public void DNFizeH(Vector<VarDecl> myLocals)
    {
        ...
        for (int i = 0; i < cll; i++) {
            Conjunct c = conjList.elementAt(i);
            Vector<VarDecl> locals_copy = new Vector<VarDecl>(myLocals.size());
    ...
    {
        Vector<Conjunct> clp = pdnf.conjList;
        int cllp = clp.size();
        for (int clip = 0; clip < cllp; clip++) {
            Conjunct pc = clp.elementAt(clip);
    ...
    private void olAssert(boolean t)
    {
        ...
        throw new scale.common.InternalError("DNF");
    }

 ...
 * FOr.
 * <p/>
 * $Id: FOr.java,v 1.13 2005-02-07 21:28:40 burrill Exp $
 * <p/>
        ...
        extends Formula
{
    ...
    public void beautify()
    {
        ...
        while (!uglies.empty()) {
            ...
            if (f.nodeType() == OP_OR) {
                ...
                Vector<Formula> v = f.myChildren;
                int cl = v.size();
                for (int i = 0; i < cl; i++) {
                    Formula c = v.elementAt(i);
                    c.setParent(null);
                    uglies.push(c);
                }
                f.delete();
    ...
    public DNF DNFize()
    {
        ...
        while (myChildren.size() != 0) {
            DNF c_dnf = myChildren.remove(0).DNFize();
    ...
    {
        Vector<Formula> mc = new Vector<Formula>();
        mc.addVectors(myChildren);

        while (mc.size() > 0) {
            Formula f = mc.remove(0);
            FExists e = addExists();

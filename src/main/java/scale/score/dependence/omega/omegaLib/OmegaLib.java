...
public final class OmegaLib
{
    ...
    private HashMap<String, CName> nameHashTable = new HashMap<String, CName>(203);
    private Vector<VarDecl> inputVars = new Vector<VarDecl>();
    private Vector<VarDecl> outputVars = new Vector<VarDecl>();
    ...
    public void initialize()
    {
        ...
        if (nameHashTable == null) {
            nameHashTable = new HashMap<String, CName>(203);
        }
        else {
            nameHashTable.clear();
        }
    ...
    public void dumpInputVars(int n)
    {
        ...
        for (int i = 1; i <= n; i++) {
            VarDecl cs = inputVars.elementAt(i);
    ...
    public void increment(VarDecl v)
    {
        ...
        CName c = nameHashTable.get(v.baseName());
    ...
    {
        CName c = nameHashTable.get(name);
        c.decrement();
    }
        ...
        for (int j = g2 % hashTableSize; true; j = (j + 1) % hashTableSize) {
            ...
            if (proto == null) {
                ...
                if (proto.getKey() > maxKeys) {
                    throw new scale.common.InternalError("too many hash keys generated ");
                }

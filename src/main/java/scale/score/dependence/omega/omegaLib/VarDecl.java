...
public final class VarDecl
{
    ...
    public int getPosition()
    {
        if ((var_kind != INPUT_VAR) && (var_kind != OUTPUT_VAR)) {
            throw new scale.common.InternalError("VarDecl.getPosition: bad var_kind " + varTypes[var_kind]);
        }
        return position;
    }
    ...
    public GlobalVarDecl getGlobalVar()
    {
        if (var_kind != GLOBAL_VAR) {
            throw new scale.common.InternalError("VarDecl::getGlobalVar: bad var_kind");
        }
        return global_var;
    }
...
//      return null;
    ...
    public String displayName(OmegaLib omegaLib)
    {
        ...
        if (primes > 200) {
            throw new scale.common.InternalError("Too many primes (" + primes + ") for " + this + " " + s.toString());
        }
    ...
    private void olAssert(boolean t)
    {
        ...
        throw new scale.common.InternalError("VarDecl");
    }

    public static Vector<VarDecl> copyVarDecls(Vector<VarDecl> vl)
    {
        int l = vl.size();
        Vector<VarDecl> new_vl = new Vector<VarDecl>(l);

        for (int p = 0; p < l; p++) {
            VarDecl v = vl.elementAt(p);
            VarDecl nv = v;

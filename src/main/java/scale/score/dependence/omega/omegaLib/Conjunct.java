        ...
        extends FDeclaration
{
    ...
    private Vector<VarDecl> mappedVars;   // List of all variables.
    ...
    public Conjunct(OmegaLib omegaLib, Formula f, RelBody r)
    {
        ...
        this.mappedVars = new Vector<VarDecl>();
        this.myLocals = new Vector<VarDecl>();
    ...
    public Conjunct(Conjunct conj)
    {
        ...
        if (conj.problem == null) {
            throw new scale.common.InternalError("problem is null");
        }

        this.mappedVars = null;
        if (conj.mappedVars != null) {
            this.mappedVars = new Vector<VarDecl>(conj.mappedVars.size());
            for (VarDecl vd : conj.mappedVars) {
                this.mappedVars.add(vd);
            }
        }
    ...
    public Conjunct copy(Formula parent, RelBody rel_body)
    {
        ...
        new_conj.mappedVars = null;
        if (mappedVars != null) {
            new_conj.mappedVars = new Vector<VarDecl>(mappedVars.size());
            for (VarDecl vd : mappedVars) {
                new_conj.mappedVars.add(vd);
            }
        }
    ...
    public boolean leadingDirValidAndKnown()
    {
        ...
        if ((leading_dir != 0) && (possible_leading_0s != guaranteed_leading_0s)) {
            throw new scale.common.InternalError("leadingDirValidAndKnown " +
                    leading_dir +
                    " " +
                    possible_leading_0s +
                    " " +
                    guaranteed_leading_0s);
        }
    ...
    public boolean hasWildcards(Equation eq)
    {
        ...
        for (int i = 1; i <= l; i++) {
            ...
            VarDecl v = mappedVars.elementAt(i - 1);
    ...
    public boolean varIsConstant(VarDecl v, Equation eq)
    {
        ...
        for (int i = 1; i <= l; i++) {
            ...
            VarDecl vc = mappedVars.elementAt(i - 1);
    ...
    private int rank()
    {
        ...
        for (int i = 0; i < l; i++) {
            VarDecl v = conj.mappedVars.elementAt(i);
    ...
    public void recomputeMappedVars()
    {
        ...
        Vector<VarDecl> new_mapped = new Vector<VarDecl>(numVars);  // This is expanded by "append"

        for (int i = 1; i <= numSafe; i++) {
            // what is now in column i used to be in column p.var[i]
            VarDecl v = mappedVars.elementAt(problem.varMappedAt(i) - 1);
            if (v.kind() == VarDecl.WILDCARD_VAR) {
                throw new scale.common.InternalError("found wild card " + v);
            }
    ...
    public void assertLeadingInfo()
    {
        ...
        if (((leading_dir != 0) && (possible_leading_0s != guaranteed_leading_0s)) ||
            ...
            throw new scale.common.InternalError("assertLeadingInfo " +
                    leading_dir +
                    " " +
                    possible_leading_0s +
                    " " +
        ...
        if ((guaranteed_leading_0s == possible_leading_0s) &&
                (possible_leading_0s >= 0) &&
                (carried_level <= min(rel.numberInput(), rel.numberOutput()))) {
            VarDecl in = rel.inputVar(carried_level);
            VarDecl out = rel.outputVar(carried_level);
            boolean guar = queryDifference(out, in, bounds);
            int lb = bounds[0];
    ...
    public String tryToPrintVarToStringWithDiv(VarDecl v)
    {
        ...
        for (int e = 0; e < neqs; e++) {
            ...
            for (int i = 1; i <= l; i++) {
                ...
                VarDecl cv = mappedVars.elementAt(i - 1);
                if (cv == v) {
                    continue;
                }
...
//        System.out.print(")");
            ...
            int l = myLocals.size();
            for (int i = 0; i < l; i++) {
                VarDecl v = myLocals.elementAt(i);
                if (i > 0) {
                    s.append(",");
                }
...
//          v.printVarAddrs(s);
            ...
            for (int i = 0; i < ml; i++) {
                VarDecl v = mappedVars.elementAt(i);
                if (i > 0) {
                    s.append(",");
                }
                s.append(v.name(omegaLib));
//          v.printVarAddrs(s);
        ...
        for (int i = 0; i < l; i++) {
            Formula c = myChildren.elementAt(i);
            c.prefixPrint(debug);
        }
    ...
    public void copyConstraint(Equation eqns, Conjunct from, Equation eq)
    {
        ...
        Vector<VarDecl> vars = from.mappedVars;
        int l = vars.size();
        ...
        for (int i = 1; i <= l; i++) {
            ...
            switch (vk) {
                ...
                case VarDecl.EXISTS_VAR:
                    throw new scale.common.InternalError("Can't copy quantified constraints across relations");
                ...
                default:
                    throw new scale.common.InternalError("copyConstraint: variable of impossible type");
            }
    ...
    public String getVarName(int col)
    {
        ...
        VarDecl v = mappedVars.elementAt(col - 1);
        olAssert(v != null);
...
//      buf.append(')');
    ...
    public int getColumn(VarDecl D)
    {
        ...
        if (col <= 0) {
            throw new scale.common.InternalError("getColumn " + col); // Not substituted
        }
        return col;
    }
    ...
    private int mapToColumn(VarDecl D)
    {
        ...
        if ((D.kind() == VarDecl.GLOBAL_VAR) && !relation().hasLocal(D.getGlobalVar(), D.functionOf())) {
            throw new scale.common.InternalError("Attempt to update global var without a local variable ID");
        }
    ...
    private void reorder()
    {
        ...
        while (first_wild < last_prot) {
            for (; first_wild <= var_no; first_wild++) {
                VarDecl v = mappedVars.elementAt(first_wild - 1);
                if (v.kind() == VarDecl.WILDCARD_VAR) {
                    break;
                }
            }
            ...
            for (; last_prot >= 1; last_prot--) {
                VarDecl v = mappedVars.elementAt(last_prot - 1);
                if (v.kind() != VarDecl.WILDCARD_VAR) {
                    break;
                }
            }
            ...
            VarDecl t = mappedVars.elementAt(first_wild - 1);
        ...
        for (safe_vars = 0; safe_vars < var_no; safe_vars++) {
            VarDecl v = mappedVars.elementAt(safe_vars);
            if (v.kind() == VarDecl.WILDCARD_VAR) {
                break;
            }
        }
        ...
        for (int s = safe_vars; s < var_no; s++) {
            VarDecl v = mappedVars.elementAt(s);
            if (v.kind() != VarDecl.WILDCARD_VAR) {
                throw new scale.common.InternalError("Not a wildcard " + v);
            }
        }
    ...
    private void sortByBaseName(Vector<VarDecl> gvars)
    {
        int gvl = gvars.size();
        for (int i = 0; i < gvl; i++) {
            for (int j = 0; j < gvl - 1; j++) {
                VarDecl vj = gvars.elementAt(j);
                VarDecl vj1 = gvars.elementAt(j + 1);
    ...
    public void reorderForPrint(boolean reverseOrder, int first_pass_input, int first_pass_output, boolean sort)
    {
        ...
        Vector<VarDecl> newpos = new Vector<VarDecl>();
        Vector<VarDecl> wcvars = new Vector<VarDecl>();
        Vector<VarDecl> gvars = new Vector<VarDecl>();
        ...
        for (int i = 1; i <= numVars; i++) {
            int col = findColumn(newpos.elementAt(i - 1)); // Find column in original conj.
            olAssert(col != 0);
    ...
    private void moveUfsToInput()
    {
        ...
        HashSet<Object> already_done = new HashSet<Object>(11);
        ...
        Vector<VarDecl> v = body.getGlobalDecls();
        int vl = v.size();
        for (int vi = 0; vi < vl; vi++) {
            VarDecl func = v.elementAt(vi);
    ...
    public void countLeadingZeros()
    {
        ...
        if (body.isSet()) {
            if ((guaranteed_leading_0s != -1) || (possible_leading_0s != -1)) {
                throw new scale.common.InternalError("countLeadingZeros");
            }
    ...
    public int cost()
    {
        ...
        for (int j = 1; j <= numVars; j++) {
            VarDecl v = mappedVars.elementAt(j - 1);
        ...
        if ((neqs == 0) && (ngeqs <= 1)) {
            if (c > 1) { // the GEQ had a wildcard -- I'm not ready to go here.
                if (omegaLib.trace) {
                    System.out.println("Refusing to negate a GEQ with wildcard(s) under " +
                            "restricted_negation; it may be possible to fix this.");
                }
    ...
    public Conjunct mergeConjuncts(Conjunct conj2, int action, RelBody body)
    {
        ...
        if (action == MERGE_COMPOSE) {
            ...
            if (conj3.guaranteed_leading_0s > conj3.possible_leading_0s) {
                throw new scale.common.InternalError("mergeConjuncts " +
                        conj3.guaranteed_leading_0s +
                        " > " +
                        conj3.possible_leading_0s);
            }
            ...
            if (conj3.leading_dir == 0) {
                conj3.possible_leading_0s = min(conj3.relation().numberInput(),
                        conj3.relation().numberOutput());
            }

            if ((conj3.guaranteed_leading_0s > conj3.possible_leading_0s) ||
                    ((conj3.guaranteed_leading_0s != conj3.possible_leading_0s) &&
                            (0 != conj3.leading_dir))) {
                throw new scale.common.InternalError("mergeConjuncts " +
                        conj3.guaranteed_leading_0s +
                        " > " +
            ...
            int redAndBlackGuarLeadingZeros = max(guaranteed_leading_0s,
            ...
            if (conj3.possible_leading_0s < redAndBlackGuarLeadingZeros) {
                ...
                if (guaranteed_leading_0s == conj2.guaranteed_leading_0s) {
                    ...
                    if (leadingDirValidAndKnown()) {
                        throw new scale.common.InternalError("leading_dir is valid and known");
                    }
                    conj3.leading_dir = conj2.leading_dir;
                }
                else {
                    if (conj2.leadingDirValidAndKnown()) {
                        throw new scale.common.InternalError("conj2.leading_dir is valid and known");
                    }
                    conj3.leading_dir = leading_dir;
                }
            }
      ...
      /* ~(ax + by + c >= 0) = (-ax -by -c-1 >= 0) */
            ...
            for (int j = 1; j <= numVars; j++) {
                ...
                if ((v.kind() == VarDecl.WILDCARD_VAR) && (cj != 0)) {
                    throw new scale.common.InternalError("negateConjunct: wildcard in inequality");
                }
        ...
        for (int i = 0; i < neqs; i++) {
            ...
            if (wc_no != 0) {
                ...
                if (problem.isNotEQZero(i, wc_j)) {
                    throw new scale.common.InternalError("Too many non-zero coefficients");
                }
    ...
    private boolean checkSubseqN(Vector<VarDecl> evac_from,
            Vector<VarDecl> evac_to,
            int n_from,
            int n_to,
            int max_arity,
            int n,
            boolean allow_offset)
    {
        ...
        for (int v = 1; v <= max_arity; v++) {
            ...
            for (int tv = 1; tv <= n_to; tv++) {
                if (tv != v + n) {
                    int col = d.findColumn(evac_to.elementAt(tv));
                    if (col > 0) {
                        prob.trytoSub(col);
                    }
                }
            ...
            for (int fv = 1; fv <= n_from; fv++) {
                if (fv != v) {
                    int col = d.findColumn(evac_from.elementAt(fv));
                    if (col > 0) {
            ...
            int c_to = d.findColumn(evac_to.elementAt(v + n));
            int c_from = d.findColumn(evac_from.elementAt(v));
    ...
    private void assertSubbedSyms()
    {
        ...
        Vector<VarDecl> gv = relation().getGlobalDecls();
        int l = gv.size();
        for (int v = 1; v <= l; v++) {
            int col = findColumn(gv.elementAt(v));
            if (col > 0) {
    ...
    private boolean checkOffset(Vector<VarDecl> evac_from, Vector<VarDecl> evac_to, int n_from, int n_to, int max_arity)
    {
        assertSubbedSyms();
    ...
    private boolean checkSubseq(Vector<VarDecl> evac_from, Vector<VarDecl> evac_to, int n_from, int n_to, int max_arity)
    {
        assertSubbedSyms();
    ...
    private boolean checkOffsetSubseq(Vector<VarDecl> evac_from, Vector<VarDecl> evac_to, int n_from, int n_to, int max_arity)
    {
        assertSubbedSyms();
    ...
    private boolean checkAffine(Vector<VarDecl> evac_from, Vector<VarDecl> evac_to, int n_from, int n_to, int max_arity)
    {
        ...
        for (int v = 1; v <= max_arity; v++) {
            int col = c.findColumn(evac_to.elementAt(v));
            if (col > 0) {
        ...
        for (int v = 1; v <= max_arity; v++) {
            if (c.findColumn(evac_to.elementAt(v)) >= 0) {
                return false;
            }
        }
    ...
    private int study(boolean in_out, RelBody rel, int max_arity)
    {
        ...
        Vector<VarDecl> evac_from = omegaLib.inputVars();
        Vector<VarDecl> evac_to = omegaLib.inputVars();
        int n_from = rel.numberInput();
        int n_to = rel.numberOutput();

        if (!in_out) {
            Vector<VarDecl> t = evac_from;
            evac_from = evac_to;
            evac_to = t;
            int x = n_from;
            n_from = n_to;
        ...
        if (queryGuaranteedLeadingZeros() >= max_arity) {
            ...
            int l = gv.size();
            for (int v = 1; v <= l; v++) {
                int col = c.findColumn(gv.elementAt(v));
                if (col > 0) {
                    c.problem.trytoSub(col);
                }
            }
    ...
    public void domain(RelBody r, int a)
    {
        ...
        for (int vi = 0; vi < l; vi++) {
            VarDecl func = mappedVars.elementAt(vi);
    ...
    public void remapDNFVars(RelBody ref_rel)
    {
        ...
        for (int j = 0; j < mvl; j++) {
            VarDecl v = mappedVars.elementAt(j);
            ...
            switch (v.kind()) {
                ...
                case VarDecl.WILDCARD_VAR:
                    break;
                default:
                    throw new scale.common.InternalError("bad variable kind");
            }
        }
    }
    ...
    public void extractNonWildVars(Vector<VarDecl> allVars)
    {
        ...
        for (int k = 0; k < vl; k++) {
            VarDecl vi = mappedVars.elementAt(k);
    ...
    public void makeLevelCarriedTo(DNF newstuff, int split_to, int level)
    {
        ...
        for (int leading_eqs = 1; leading_eqs <= split_to; leading_eqs++) {
            ...
            Vector<VarDecl> vb = body.getGlobalDecls();
            int vbl = vb.size();
            for (int fi = 0; fi < vbl; fi++) {
                VarDecl func = vb.elementAt(fi);
                GlobalVarDecl f = func.getGlobalVar();
    ...
    public boolean parallel(Equation eq, Conjunct that, Equation eqt)
    {
        ...
        Vector<VarDecl> varst = that.variables();
        int lt = varst.size();
    ...
    public void farkas()
    {
        ...
        Vector<VarDecl> vl = variables();
        int vll = vl.size();
        for (int vli = 0; vli < vll; vli++) {
            VarDecl v1 = vl.elementAt(vli);
            int col1 = findColumn(v1);
            if (col1 == 0) {
                throw new scale.common.InternalError("Variable not found");
            }

        ...
        extends FDeclaration
{
    ...
    public FExists(OmegaLib omegaLib, Formula f, RelBody b, Vector<VarDecl> vid)
    {
    ...
    public Conjunct findAvailableConjunct()
    {
        ...
        return myChildren.elementAt(0).findAvailableConjunct();
    }
    ...
    public void beautify()
    {
        ...
        if (myLocals.size() == 0) {
            ...
            parent().checkAndAddChild(myChildren.remove(0));
            delete();
        }
        else {
            Formula child = myChildren.elementAt(0);
    ...
    public DNF DNFize()
    {
        DNF dnf = myChildren.remove(0).DNFize();

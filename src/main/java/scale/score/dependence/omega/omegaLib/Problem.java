...
public final class Problem
{
  ...
  /* Allocation parameters and functions */
    ...
    public Problem(OmegaLib omegaLib, int in_eqs, int in_geqs, Conjunct varNameSource)
    {
        ...
        this.safeVars = 0;
        this.redMemory = new Vector<RememberRedConstraint>();
    ...
    public Problem(Problem p2, Conjunct varNameSource)
    {
        ...
        this.var = p2.var.clone();
        this.forwardingAddress = p2.forwardingAddress.clone();
        this.varNameSource = varNameSource;
    ...
    private int checkMultiply(long x, long y)
    {
        ...
        int r = (int) p;
        if (r != p) {
            throw new scale.common.InternalError("integer multiply overflow " + x + " " + y);
        }
        return r;
    }
    ...
    {
        if (b <= 0) {
            throw new scale.common.InternalError("integer divide error " + a + " " + b);
        }
    ...
    {
        if ((a < 0) || (b < 0)) {
            throw new scale.common.InternalError("gcd error " + a + " " + b);
        }
    ...
    {
        if ((a < 0) || (b < 0)) {
            throw new scale.common.InternalError("lcm error " + a + " " + b);
        }
        ...
        if (verify) {
            ...
            if (nEQs > 0) { // found some equality constraints during verification
                ...
                if ((mayBeRed != 0) && (nVars == safeVars) && (numRed == 1)) {
                    ...
                    throw new scale.common.InternalError("Added equality constraint to verified problem generates false");
                }
            }
        }
...
//      context.check();
    ...
    private void substitute(Equation sub, int i, int c)
    {
        ...
        if ((i > safeVars) && (clr != 0)) {
            ...
            if (unsafeSub) {
                ...
                System.out.println("");
                printProblem();
                throw new scale.common.InternalError("UNSAFE RED SUBSTITUTION");
            }
        }
    ...
    public void printProblem(boolean debug)
    {
        ...
        for (int e = 0; e < nMemories; e++) {
            RememberRedConstraint rm = redMemory.elementAt(e);
            omegaLib.printHead();
    ...
    {
        if (n < 0) {
            throw new scale.common.InternalError("ERROR: nEQs < 0??");
        }
    }
...
//    }
    ...
    public boolean queryVariableBounds(int i, int[] bounds)
    {
        ...
        if ((abs(forwardingAddress[i]) == 1) && ((nVars + nSUBs) == 2) && ((nEQs + nSUBs) == 1)) {
            queryCoupledVariable(i, bounds, bounds.clone());
            return false;
        }
        return true;
    }
    ...
    private boolean queryCoupledVariable(int i, int[] bounds, int[] oldBounds)
    {
        ...
        if ((abs(forwardingAddress[i]) != 1) || ((nVars + nSUBs) != 2) || ((nEQs + nSUBs) != 1)) {
            throw new scale.common.InternalError("queryCoupledVariablecalled with bad parameters " + this);
        }
    ...
    private boolean chainKill(int color, boolean onlyWildcards)
    {
        ...
        boolean[] tryToEliminate = new boolean[nGEQs + 1];
        boolean[] redundant = new boolean[nGEQs + 1];
                    ...
                    for (int e3I = e3U; e3I >= e3L; e3I--) {
                        ...
                        if (alpha3 > 0) { /* Trying to prove e3 is redundant */
                            ...
                            for (int k = nVars; k >= 1; k--) {
                                if (checkMultiply(alpha3,
                                        eqe3.getCoefficient(k)) != checkMultiply(alpha1, eqe1.getCoefficient(k)) +
                                        checkMultiply(alpha2, eqe2.getCoefficient(k))) {
                                    continue nextE3;
                                }
                            }
                        ...
                        for (int k = nVars; k >= 1; k--) {
                            if ((alpha3 * eqe3.getCoefficient(k)) !=
                                    (alpha1 * eqe1.getCoefficient(k) + alpha2 * eqe2.getCoefficient(k))) {
                                continue nextE3;
                            }
                        }
...
//    }
        ...
        for (int e = nEQs - 1; e >= 0; e--) {
            ...
            for (int i = nVars; i >= safeVars + 1; i--) {
                ...
                for (int e2 = nEQs - 1; e2 >= 0; e2--) {
                    ...
                    preserveThisConstraint = (preserveThisConstraint &&
                            gcd(a, abs(eqe2.getCoefficient(i))) != 1);
    ...
    private void check()
    {
        ...
        if (mayBeRed == 0) {
            if (anyNotBlack()) {
                throw new scale.common.InternalError("check - some not BLACK");
            }
            return;
        }
...
//      }
    ...
    {
        if ((type == RememberRedConstraint.redEQ) &&
                (omegaLib.newVar == nVars) &&
                (e.getCoefficient(omegaLib.newVar) != 0)) { // this is really a stride constraint
            type = RememberRedConstraint.redStride;
            stride = e.getCoefficient(omegaLib.newVar);
        }
        else if (e.anyNZCoef(nVars, safeVars + 1)) {
            throw new scale.common.InternalError("Non-zero coefficient in rememberRedConstraint");
        }
        ...
        if (omegaLib.trace) {
            ...
            switch (type) {
                case RememberRedConstraint.notRed:
                    System.out.print("notRed: ");
                    break;
                case RememberRedConstraint.redGEQ:
                    System.out.print("Red: 0 <= ");
                    break;
                case RememberRedConstraint.redLEQ:
                    System.out.print("Red: 0 >= ");
                    break;
                case RememberRedConstraint.redEQ:
                    System.out.print("Red: 0 == ");
                    break;
                case RememberRedConstraint.redStride:
                    System.out.print("Red stride " + stride + ": ");
                    break;
            }
            printTerm(e, 1);
            System.out.println("");
        ...
        if (anyWild) {
            ...
            for (int i = 1; i <= safeVars; i++) {
                if ((var[i] < 0) && (e.isNotZero(i))) {
                    if (backSub[i] < 0) {
                        printProblem();
                        throw new scale.common.NotImplementedError("sophisticated back substitutions not handled ");
                    }
                    int j = backSub[i];
    ...
    private void recallRedMemories()
    {
        ...
        for (int m = 0; m < nMemories; m++) {
            RememberRedConstraint redmem = redMemory.elementAt(m);
    ...
    private int solve(int desiredResult)
    {
        ...
        if (solveDepth > 50) {
            ...
            if (solveDepth > 60) {
                throw new scale.common.InternalError("Solve depth > 60");
            }
        }
                ...
                for (int e2 = nGEQs - 1; e2 >= 0; e2--) {
                    ...
                    if (g == 0) {
                        if (diff < 0) {
                            ...
                            throw new scale.common.InternalError("Found conflicting constraints ");
                        }
                        if (diff < grey) {
    ...
    private int solveGEQ(int desiredResult)
    {
        ...
        while (true) {
            ...
            if (omegaLib.trace) {
                System.out.print("\nSolveGEQ(");
                System.out.print(solveDisp[desiredResult]);
                System.out.print((omegaLib.pleaseNoEqualitiesInSimplifiedProblems ? ", No EQs" : ""));
                System.out.println("):");
                printProblem();
                System.out.println("");
            }
            ...
            for (int e = 0; e < nSUBs; e++) {
                if (SUBs[e].anyNZCoef(nVars, safeVars + 1)) {
                    throw new scale.common.InternalError("non-zero wildcard coefficient.");
                }
            }
          ...
          /* our equation is ax + c >= 0, or ax >= -c, or c >= -ax */
            ...
            switch (normalize()) {
                ...
                case normalizeUncoupled:
                    ...
                    break;
                default:
                    throw new scale.common.InternalError("impossible case in SolveGEQ");
            }
          ...
          /* sort array */
                    ...
                    for (int j = 0; j < lowerBounds; j++) {
                        int e = lowerBound[j];
                        Equation eqe = GEQs[e];
                        int maxIncr = ((eqe.getCoefficient(i) - 1) *
                                (worstLowerBoundConstant - 1) - 1) / worstLowerBoundConstant;
...
//    }
   ...
   */
    ...
    public void merge(int action, Conjunct conj1, Conjunct conj2, Conjunct conj3, Vector<VarDecl> newVars)
    {
        ...
        int new_col = 1;
        Vector<VarDecl> vars1 = conj1.variables();
        int l1 = vars1.size();
        for (int i = 1; i <= l1; i++) {
            VarDecl v = vars1.elementAt(i - 1);
        ...
        Vector<VarDecl> vars2 = conj2.variables();
        int l2 = vars2.size();
        for (int i = 1; i <= l2; i++) {
            VarDecl v = vars2.elementAt(i - 1);
            if (v.kind() == VarDecl.WILDCARD_VAR) {
                continue;
            }
    ...
    public int findColumn(int column)
    {
        ...
        if (column > nVars) {
            throw new scale.common.InternalError("col " + column + " > " + nVars);
        }
    ...
    public boolean trytoSub(int col)
    {
        ...
        if (eq.anyNZCoef(nVars, 0)) {
            throw new scale.common.InternalError("Non-zero coefficient");
        }

...
public class TypeTable
{
    private static class Entry
    {
        ...
        public Type value; /* The key maps to this Type */
        public int flag;  /* A flag value. */
        public Entry next;  /* The next entry with the same hash code. */

        public Entry(long key, Type value, int flag, Entry next)
        {
            this.key = key;
            this.value = value;
            this.flag = flag;
     ...
     */
    public synchronized Type put(long key, Type value, int flag)
    {
        int index = hash(key);
     ...
     * @return the value
     */
    public synchronized Type get(long key)
    {
        int index = hash(key);
        Entry entry = map[index];
        while (entry != null) {
    ...
    private class TypeTableEnum
            implements Enumeration<Type>
    {
        int index = -1;
        Entry next = null;

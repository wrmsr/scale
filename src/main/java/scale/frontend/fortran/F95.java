        ...
        extends Parser
{
     ...
     * The token provides the filename, line number and column.
     */
    private void userError(int errno, String text1, int lineNumber, int column)
            throws InvalidException
    {
     ...
     * The token provides the filename, line number and column.
     */
    private void userError(int errno, String text)
            throws InvalidException
    {
     ...
     * The token provides the filename, line number and column.
     */
    private void userError(int errno, String text1, String text2)
            throws InvalidException
    {
        userError(errno, text1, text2, null, null, lineNumber, column);
    }
     ...
     * The token provides the filename, line number and column.
     */
    private void notImplementedError(String msg, int lineNumber, int column)
            throws InvalidException
    {
        fatalError = true;
     ...
     * The token provides the filename, line number and column.
     */
    private void notImplementedError(String msg)
            throws InvalidException
    {
        fatalError = true;
        Msg.reportError(Msg.MSG_Not_implemented_s, filename, lineNumber, column, msg);
     ...
     */
    private FileReader newFile(String filename)
            throws IOException
    {
    ...
    private Type charToType(char c)
    {
        switch (c) {
            ...
            case 'Z':
                return PointerType.create(float_complex_type);
            case 'x':
                return double_complex_type;
            case 'X':
                return PointerType.create(double_complex_type);
            default:
                throw new scale.common.InternalError("Unknown type char(" + c + ").");
        }
    }
    ...
    private Statement makeCallExitStmt(Expression arg)
            throws InvalidException
    {
     ...
     */
    private Expression cast(Type type, Expression exp, int lineNumber, int column)
            throws InvalidException
    {
        if (fatalError) {
            return errExp;
     ...
     * Generate a cast expression.
     */
    private Expression cast(Type type, Expression exp)
            throws InvalidException
    {
     ...
     */
    private boolean nextCharMustBe(char c)
            throws InvalidException
    {
     ...
     */
    private boolean nextNBCharMustBe(char c)
            throws InvalidException
    {
        while (statement[column] == ' ') {
            column++;
    ...
    private VariableDecl nextVariable()
            throws InvalidException
    {
    ...
    private EquivalenceDecl nextEqVariable(VariableDecl base)
            throws InvalidException
    {
     ...
     * not found.
     */
    private Literal nextArrayConstant()
            throws InvalidException
    {
        if (!allowF90Features) {
            return null;
    ...
    private ProcedureDecl nextDefinedBinaryOp()
            throws InvalidException
    {
        return null;
    }

    private ProcedureDecl nextDefinedUnaryOp()
            throws InvalidException
    {
        return null;
    }

    private Expression nextArrayExpr()
            throws InvalidException
    {
     ...
     * right-paren.
     */
    private Vector<Expression> nextArgumentList()
            throws InvalidException
    {
     ...
     * right-paren.
     */
    private Vector<Expression> nextSubscriptList()
            throws InvalidException
    {
     ...
     */
    private Type getArraySubtype(ArrayType at, Vector<Expression> args)
            throws InvalidException
    {
        int l = args.size();
        int rank = at.getRank();
     ...
     */
    private Expression buildNewFtnCall(String name, Type returnType, Vector<Expression> args)
            throws InvalidException
    {
        Declaration decl = lookupDecl(name);
        if (decl == null) {
     ...
     */
    private Expression genMax(int exprType, Type rtype, Type atype, Vector<Expression> args)
            throws InvalidException
    {
     ...
     */
    private Expression genMin(int exprType, Type rtype, Type atype, Vector<Expression> args)
            throws InvalidException
    {
        Expression res = args.get(0);
        int l = args.size();
     ...
     *
     */
    private Literal genSelIntKind(Vector<Expression> args)
            throws InvalidException
    {
        if (!allowF90Features) {
            return null;
    ...
    private Expression genBTEST(Expression arg1, Expression arg2)
            throws InvalidException
    {
     ...
     *
     */
    private Literal genSelRealKind(Vector<Expression> args)
            throws InvalidException
    {
        if (!allowF90Features) {
            return null;
    ...
    private Expression buildIndexCall(Expression arg1, Expression arg2)
            throws InvalidException
    {
    ...
    private Expression nextIntrinsicCall(String name, int exprType)
            throws InvalidException
    {
        ...
        try {
            ...
            switch (ftn) {
                ...
                case Intrinsics.BTEST:
                    call = genBTEST(arg1, arg2);
                    break;
                case Intrinsics.AMAX0:
                    call = genMax(exprType, real_type, int_type, args);
                    break;
                case Intrinsics.AMAX1:
                    call = genMax(exprType, real_type, real_type, args);
                    break;
                case Intrinsics.AMIN0:
                    call = genMin(exprType, int_type, real_type, args);
                    break;
                case Intrinsics.AMIN1:
                    call = genMin(exprType, real_type, real_type, args);
                    break;
                case Intrinsics.AMOD:
                    ...
                    break;
                case Intrinsics.CMPLX:
                    call = genCMPLX(arg1, arg2, float_complex_type);
                    break;
                case Intrinsics.DCMPLX:
                    call = genCMPLX(arg1, arg2, double_complex_type);
                    break;
                ...
                case Intrinsics.DIM:
                    call = buildScalarDyadicOp(exprType, DIM_OP, arg1, arg2);
                    break;
                case Intrinsics.DMAX1:
                    call = genMax(exprType, double_type, double_type, args);
                    break;
                case Intrinsics.DMIN1:
                    call = genMin(exprType, double_type, double_type, args);
                    break;
                case Intrinsics.DMOD:
                    call = buildScalarDyadicOp(exprType, REM_OP, arg1, arg2);
                    break;
                ...
                case Intrinsics.IOR:
                    ...
                    call = buildScalarDyadicOp(exprType, OR_OP, arg1, arg2);
                    break;
                case Intrinsics.ISHFT:
                    call = genISHFT(arg1, arg2, false);
                    break;
                case Intrinsics.ISHFTC:
                    call = genISHFT(arg1, arg2, true);
                    break;
                case Intrinsics.ISIGN:
                    call = buildScalarDyadicOp(exprType, SIGN_OP, arg1, arg2);
                    break;
                case Intrinsics.MAX:
                    call = genMax(exprType, t1, t1, args);
                    break;
                case Intrinsics.MAX0:
                    call = genMax(exprType, int_type, int_type, args);
                    break;
                case Intrinsics.MAX1:
                    call = genMax(exprType, int_type, real_type, args);
                    break;
                case Intrinsics.MIN:
                    call = genMin(exprType, t1, t1, args);
                    break;
                case Intrinsics.MIN0:
                    call = genMin(exprType, int_type, int_type, args);
                    break;
                case Intrinsics.MIN1:
                    call = genMin(exprType, int_type, real_type, args);
                    break;
                case Intrinsics.MOD:
                    call = buildScalarDyadicOp(exprType, REM_OP, arg1, arg2);
                    break;
                case Intrinsics.SIGN:
                    call = buildScalarDyadicOp(exprType, SIGN_OP, arg1, arg2);
                    break;
                ...
                case Intrinsics.ABS:
                    ...
                    break;
                case Intrinsics.ACOS:
                    call = buildScalarMonadicOp(exprType, ACOS_OP, t1, arg1);
                    break;
                case Intrinsics.AIMAG:
                    call = buildScalarMonadicOp(exprType, IMAG_OP, real_type, arg1);
                    break;
                case Intrinsics.AINT:
                    call = buildScalarMonadicOp(exprType, FLOOR_OP, t1, arg1);
                    break;
                case Intrinsics.ALOG10:
                    call = buildScalarMonadicOp(exprType, LOG10_OP, real_type, arg1);
                    break;
                case Intrinsics.ALOG:
                    call = buildScalarMonadicOp(exprType, LOG_OP, real_type, arg1);
                    break;
                case Intrinsics.ANINT:
                    call = buildScalarMonadicOp(exprType, ROUND_OP, t1, arg1);
                    break;
                case Intrinsics.ASIN:
                    call = buildScalarMonadicOp(exprType, ASIN_OP, t1, arg1);
                    break;
                case Intrinsics.ATAN:
                    call = buildScalarMonadicOp(exprType, ATAN_OP, t1, arg1);
                    break;
                case Intrinsics.CABS:
                    call = buildScalarMonadicOp(exprType, ABS_OP, real_type, arg1);
                    break;
                case Intrinsics.CCOS:
                    call = buildScalarMonadicOp(exprType, COS_OP, t1, arg1);
                    break;
                case Intrinsics.CDABS:
                    call = buildScalarMonadicOp(exprType, ABS_OP, double_type, arg1);
                    break;
                case Intrinsics.CEILING:
                    call = buildScalarMonadicOp(exprType, CEILING_OP, float_complex_type, arg1);
                    break;
                case Intrinsics.CEXP:
                    call = buildScalarMonadicOp(exprType, EXP_OP, t1, arg1);
                    break;
                case Intrinsics.CHAR:
                    call = buildScalarMonadicOp(exprType, TRUNCATE_OP, char_type, arg1);
                    break;
                case Intrinsics.CLOG:
                    call = buildScalarMonadicOp(exprType, LOG_OP, t1, arg1);
                    break;
                case Intrinsics.CONJG:
                    call = buildScalarMonadicOp(exprType, CONJG_OP, t1, arg1);
                    break;
                case Intrinsics.COS:
                    call = buildScalarMonadicOp(exprType, COS_OP, t1, arg1);
                    break;
                case Intrinsics.COSH:
                    call = buildScalarMonadicOp(exprType, COSH_OP, t1, arg1);
                    break;
                case Intrinsics.CSIN:
                    call = buildScalarMonadicOp(exprType, SIN_OP, t1, arg1);
                    break;
                case Intrinsics.CSQRT:
                    call = buildScalarMonadicOp(exprType, SQRT_OP, t1, arg1);
                    break;
                case Intrinsics.DABS:
                    call = buildScalarMonadicOp(exprType, ABS_OP, double_type, arg1);
                    break;
                case Intrinsics.DACOS:
                    call = buildScalarMonadicOp(exprType, ACOS_OP, double_type, arg1);
                    break;
                case Intrinsics.DASIN:
                    call = buildScalarMonadicOp(exprType, ASIN_OP, double_type, arg1);
                    break;
                case Intrinsics.DATAN:
                    call = buildScalarMonadicOp(exprType, ATAN_OP, double_type, arg1);
                    break;
                case Intrinsics.DBLE:
                    call = buildScalarMonadicOp(exprType, NOP_OP, double_type, arg1);
                    break;
                case Intrinsics.DCONJG:
                    call = buildScalarMonadicOp(exprType, CONJG_OP, double_complex_type, arg1);
                    break;
                case Intrinsics.DCOS:
                    call = buildScalarMonadicOp(exprType, COS_OP, double_type, arg1);
                    break;
                case Intrinsics.DCOSH:
                    call = buildScalarMonadicOp(exprType, COSH_OP, double_type, arg1);
                    break;
                case Intrinsics.DEXP:
                    call = buildScalarMonadicOp(exprType, EXP_OP, double_type, arg1);
                    break;
                case Intrinsics.DFLOAT:
                    call = buildScalarMonadicOp(exprType, NOP_OP, double_type, arg1);
                    break;
                case Intrinsics.DIMAG:
                    call = buildScalarMonadicOp(exprType, IMAG_OP, double_type, arg1);
                    break;
                case Intrinsics.DINT:
                    call = buildScalarMonadicOp(exprType, FLOOR_OP, double_type, arg1);
                    break;
                case Intrinsics.DLOG10:
                    call = buildScalarMonadicOp(exprType, LOG10_OP, double_type, arg1);
                    break;
                case Intrinsics.DLOG:
                    call = buildScalarMonadicOp(exprType, LOG_OP, double_type, arg1);
                    break;
                case Intrinsics.DNINT:
                    call = buildScalarMonadicOp(exprType, ROUND_OP, double_type, arg1);
                    break;
                case Intrinsics.DSIN:
                    call = buildScalarMonadicOp(exprType, SIN_OP, double_type, arg1);
                    break;
                case Intrinsics.DSINH:
                    call = buildScalarMonadicOp(exprType, SINH_OP, double_type, arg1);
                    break;
                case Intrinsics.DSQRT:
                    call = buildScalarMonadicOp(exprType, SQRT_OP, double_type, arg1);
                    break;
                case Intrinsics.DTAN:
                    call = buildScalarMonadicOp(exprType, TAN_OP, double_type, arg1);
                    break;
                case Intrinsics.DTANH:
                    call = buildScalarMonadicOp(exprType, TANH_OP, double_type, arg1);
                    break;
                case Intrinsics.EXP:
                    call = buildScalarMonadicOp(exprType, EXP_OP, t1, arg1);
                    break;
                case Intrinsics.FLOAT:
                    call = buildScalarMonadicOp(exprType, NOP_OP, real_type, arg1);
                    break;
                case Intrinsics.FLOOR:
                    call = buildScalarMonadicOp(exprType, FLOOR_OP, t1, arg1);
                    break;
                case Intrinsics.IABS:
                    call = buildScalarMonadicOp(exprType, ABS_OP, t1, arg1);
                    break;
                case Intrinsics.ICHAR:
                    call = buildScalarMonadicOp(exprType, TRUNCATE_OP, int_type, arg1);
                    break;
                case Intrinsics.IDFIX:
                    call = buildScalarMonadicOp(exprType, TRUNCATE_OP, int_type, arg1);
                    break;
                case Intrinsics.IDINT:
                    call = buildScalarMonadicOp(exprType, TRUNCATE_OP, int_type, arg1);
                    break;
                case Intrinsics.IDNINT:
                    call = buildScalarMonadicOp(exprType, ROUND_OP, int_type, arg1);
                    break;
                case Intrinsics.IFIX:
                    call = buildScalarMonadicOp(exprType, TRUNCATE_OP, int_type, arg1);
                    break;
                case Intrinsics.INT:
                    call = buildScalarMonadicOp(exprType, TRUNCATE_OP, int_type, arg1);
                    break;
                case Intrinsics.LOG10:
                    call = buildScalarMonadicOp(exprType, LOG10_OP, t1, arg1);
                    break;
                case Intrinsics.LOG:
                    call = buildScalarMonadicOp(exprType, LOG_OP, t1, arg1);
                    break;
                case Intrinsics.NINT:
                    call = buildScalarMonadicOp(exprType, ROUND_OP, int_type, arg1);
                    break;
                case Intrinsics.REAL:
                    call = buildScalarMonadicOp(exprType, NOP_OP, real_type, arg1);
                    break;
                case Intrinsics.SIN:
                    call = buildScalarMonadicOp(exprType, SIN_OP, t1, arg1);
                    break;
                case Intrinsics.SINH:
                    call = buildScalarMonadicOp(exprType, SINH_OP, t1, arg1);
                    break;
                case Intrinsics.SNGL:
                    call = buildScalarMonadicOp(exprType, NOP_OP, real_type, arg1);
                    break;
                case Intrinsics.SQRT:
                    ...
                    break;
                case Intrinsics.TAN:
                    call = buildScalarMonadicOp(exprType, TAN_OP, t1, arg1);
                    break;
                case Intrinsics.TANH:
                    call = buildScalarMonadicOp(exprType, TANH_OP, t1, arg1);
                    break;

                // Miscelanneous

                case Intrinsics.ACHAR:
                    break;
                case Intrinsics.ADJUSTL:
                    break;
                case Intrinsics.ADJUSTR:
                    break;
                case Intrinsics.ALL:
                    break;
                case Intrinsics.ALLOCATED:
                    break;
                case Intrinsics.ANY:
                    break;
                case Intrinsics.ASSOCIATED:
                    break;
                case Intrinsics.BIT_SIZE:
                    break;
                case Intrinsics.COUNT:
                    break;
                case Intrinsics.CSHIFT:
                    break;
                case Intrinsics.DIGITS:
                    break;
                case Intrinsics.DOT_PRODUCT:
                    break;
                case Intrinsics.EOSHIFT:
                    break;
                case Intrinsics.EPSILON:
                    break;
                case Intrinsics.EXPONENT:
                    break;
                case Intrinsics.FRACTION:
                    break;
                case Intrinsics.HUGE:
                    break;
                case Intrinsics.IACHAR:
                    break;
                case Intrinsics.IBITS:
                    break;
                case Intrinsics.KIND:
                    break;
                case Intrinsics.LBOUND:
                    break;
                case Intrinsics.LEN:
                    call = getStringLength(arg1);
                    break;
                case Intrinsics.LEN_TRIM:
                    break;
                case Intrinsics.LGE:
                    break;
                case Intrinsics.LGT:
                ...
                case Intrinsics.LLT:
                    break;
                case Intrinsics.LOGICAL:
                    break;
                case Intrinsics.MATMUL:
                    break;
                case Intrinsics.MAXEXPONENT:
                    break;
                case Intrinsics.MAXLOC:
                    break;
                case Intrinsics.MAXVAL:
                    break;
                case Intrinsics.MERGE:
                    break;
                case Intrinsics.MINEXPONENT:
                    break;
                case Intrinsics.MINLOC:
                    break;
                case Intrinsics.MINVAL:
                    break;
                case Intrinsics.MODULO:
                    break;
                case Intrinsics.NEAREST:
                    break;
                case Intrinsics.NOT:
                    break;
                case Intrinsics.NULL:
                    break;
                case Intrinsics.PACK:
                    break;
                case Intrinsics.PRECISION:
                    break;
                case Intrinsics.PRESENT:
                    break;
                case Intrinsics.PRODUCT:
                    break;
                case Intrinsics.RADIX:
                    break;
                case Intrinsics.RANGE:
                    break;
                case Intrinsics.REPEAT:
                    break;
                case Intrinsics.RESHAPE:
                    break;
                case Intrinsics.RRSPACING:
                    break;
                case Intrinsics.SCALE:
                    break;
                case Intrinsics.SCAN:
                    break;
                case Intrinsics.SELECTED_INT_KIND:
                    call = genSelIntKind(args);
                    break;
                case Intrinsics.SELECTED_REAL_KIND:
                    call = genSelRealKind(args);
                    break;
                case Intrinsics.SET_EXPONENT:
                    break;
                case Intrinsics.SHAPE:
                    break;
                case Intrinsics.SIZE:
                    break;
                case Intrinsics.SPACING:
                    break;
                case Intrinsics.SPREAD:
                    break;
                case Intrinsics.SUM:
                    break;
                case Intrinsics.TINY:
                    break;
                case Intrinsics.TRANSFER:
                    break;
                case Intrinsics.TRANSPOSE:
                    break;
                case Intrinsics.UBOUND:
                    break;
                case Intrinsics.UNPACK:
                    break;
                case Intrinsics.VERIFY:
                    break;
            }
    ...
    private ProcedureDecl defIntrinsicFtn(String name)
            throws InvalidException
    {
        int ftn = Intrinsics.lookup(name, name.length() - 1); // Drop the '_'.
        if (ftn == Intrinsics.NA) {
     ...
     */
    private Expression makeLValue(Expression expr)
            throws scale.common.InvalidException
    {
        if (fatalError) {
            return errExp;
        }
     ...
     */
    private void makeByReference(Vector<Expression> args, ProcedureType pt)
            throws InvalidException
    {
        if (fatalError) {
            return;
    ...
    private Expression nextIDExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression getArrayDimensionSize(ArrayType at, int dimension, Expression array)
            throws InvalidException
    {
        if (at.isFixedArrayType()) {
     ...
     * length.
     */
    private Type getFortranCharType(long length)
            throws InvalidException
    {
    ...
    private Expression nextPrimaryExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression convertArrayRef(ArrayType at, Expression array)
            throws InvalidException
    {
        if (arrayExprType == null) {
            arrayExprType = at;
            ...
            throws InvalidException
    {
        ...
        switch (op) {
            case ADD_OP:
                return new AdditionOp(type, la, ra);
            case SUB_OP:
            ...
            case POW_OP:
                return new ExponentiationOp(type, la, ra);
            case DIM_OP:
                return new Transcendental2Op(type, la, ra, Transcendental2Op.cDim);
            case SIGN_OP:
                return new Transcendental2Op(type, la, ra, Transcendental2Op.cSign);
            case ATAN2_OP:
                return new Transcendental2Op(type, la, ra, Transcendental2Op.cAtan2);
            case MAX_OP:
                return new MaximumOp(type, la, ra);
            case MIN_OP:
                return new MinimumOp(type, la, ra);
            case AND_OP:
                return new BitAndOp(type, la, ra);
            case OR_OP:
                return new BitOrOp(type, la, ra);
            case XOR_OP:
                return new BitXorOp(type, la, ra);
            case SFTLFT_OP:
                return new BitShiftOp(type, la, ra, ShiftMode.Left);
            default:
            ...
            throws InvalidException
    {
        ...
        switch (op) {
            case NOP_OP:
                return arg;
            case ABS_OP:
                return new AbsoluteValueOp(type, arg);
            case CONJG_OP:
                return new TranscendentalOp(type, arg, TransFtn.Conjg);
            case COS_OP:
                return new TranscendentalOp(type, arg, TransFtn.Cos);
            case COSH_OP:
                return new TranscendentalOp(type, arg, TransFtn.Cosh);
            case SIN_OP:
                return new TranscendentalOp(type, arg, TransFtn.Sin);
            case SINH_OP:
                return new TranscendentalOp(type, arg, TransFtn.Sinh);
            case TAN_OP:
                return new TranscendentalOp(type, arg, TransFtn.Tan);
            case TANH_OP:
                return new TranscendentalOp(type, arg, TransFtn.Tanh);
            case SQRT_OP:
                return new TranscendentalOp(type, arg, TransFtn.Sqrt);
            case ACOS_OP:
                return new TranscendentalOp(type, arg, TransFtn.Acos);
            case ASIN_OP:
                return new TranscendentalOp(type, arg, TransFtn.Asin);
            case ATAN_OP:
                return new TranscendentalOp(type, arg, TransFtn.Atan);
            case EXP_OP:
                return new TranscendentalOp(type, arg, TransFtn.Exp);
            case LOG10_OP:
                return new TranscendentalOp(type, arg, TransFtn.Log10);
            case LOG_OP:
                return new TranscendentalOp(type, arg, TransFtn.Log);
            case FLOOR_OP:
                return new TypeConversionOp(type, arg, CastMode.FLOOR);
            case CEILING_OP:
                return new TypeConversionOp(type, arg, CastMode.CEILING);
            case ROUND_OP:
                return new TypeConversionOp(type, arg, CastMode.ROUND);
            case IMAG_OP:
                return new TypeConversionOp(type, arg, CastMode.IMAGINARY);
            case TRUNCATE_OP:
                return new TypeConversionOp(type, arg, CastMode.TRUNCATE);
            case REAL_OP:
                return new TypeConversionOp(type, arg, CastMode.REAL);
            case COMP_OP:
                return new BitComplementOp(type, arg);
            default:
                throw new InvalidException("mult expr");
        }
    }

    private void checkArraysCompatible(ArrayType lat, ArrayType rat)
            throws InvalidException
    {
    ...
    private Expression nextLevel1Expr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextPowerExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextMultExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextLevel2Expr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextLevel3Expr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextLevel4Expr(int exprType)
            throws InvalidException
    {
        ...
        try {
            ...
            while (true) {
                ...
                if (cl || cr) {
                    ...
                    switch (op) {
                        default:
                            ...
                            break;
                        case DOT_EQ:
                            expl = new AndConditionalOp(logical_type,
                            ...
                            continue;
                        case DOT_NEQ:
                            expl = new OrConditionalOp(logical_type,
                ...
                switch (op) {
                    case DOT_GT:
                        expl = new GreaterOp(logical_type, expl, expr);
                        break;
                    case DOT_GE:
                        expl = new GreaterEqualOp(logical_type, expl, expr);
                        break;
                    case DOT_LT:
                        expl = new LessOp(logical_type, expl, expr);
                        break;
                    case DOT_LE:
                        expl = new LessEqualOp(logical_type, expl, expr);
                        break;
                    case DOT_NEQ:
    ...
    private Expression nextAndOperandExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextOrOperandExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextEqvOperandExpr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextLevel5Expr(int exprType)
            throws InvalidException
    {
    ...
    private Expression nextExpression(int exprType)
            throws InvalidException
    {
    ...
    private Literal nextConstantExpression(Type type)
            throws InvalidException
    {
    ...
    private Literal nextConstantPrimaryExpr()
            throws InvalidException
    {
    ...
    private IntLiteral nextIntegerConstantExpression()
            throws InvalidException
    {
    ...
    private Expression nextScalarExpression()
            throws InvalidException
    {
    ...
    private Expression nextIntegerExpression()
            throws InvalidException
    {
    ...
    private Expression nextSpecificationExpression()
            throws InvalidException
    {
    ...
    private Expression nextCharacterExpression()
            throws InvalidException
    {
    ...
    private Expression nextLogicalExpression()
            throws InvalidException
    {
     ...
     */
    private Vector<FormalDecl> nextFormals(boolean subroutine)
            throws InvalidException
    {
     ...
     */
    private Vector<String> nextList()
            throws InvalidException
    {
    ...
    private Expression nextUnitSpecifier()
            throws InvalidException
    {
    ...
    private Expression nextFormatSpecifier()
            throws InvalidException
    {
    ...
    private void nextCilistValues(int allowedKeywords)
            throws InvalidException
    {
    ...
    private void nextFioListValues(int allowedKeywords)
            throws InvalidException
    {
    ...
    private void nextIOList(int ioType)
            throws InvalidException
    {
        ...
        try {
            do {
                ...
                if (exp != null) {
                    switch (ioType) {
                        case DO_LIO:
                            buildDolio(exp, currentBlockStmt);
                            break;
                        case DO_FIO:
                            buildDofio(exp, currentBlockStmt);
                            break;
                        case DO_UIO:
                            buildDouio(exp, currentBlockStmt);
                            break;
                    }
                    continue;
                }
     ...
     */
    private Statement buildIOListEnd(String ftn)
            throws InvalidException
    {
     ...
     */
    private Statement buildAlistFtn(String ftn)
            throws InvalidException
    {
     ...
     */
    public void callback(int callback, Expression exp, BlockStmt bs)
            throws InvalidException
    {
        switch (callback) {
            case DO_LIO:
            ...
            case DO_UIO:
                ...
                break;
            default:
                throw new scale.common.InternalError("Unknown callback.");
        }
     ...
     */
    private void buildDolio(Expression exp, BlockStmt bs)
            throws InvalidException
    {
        BlockStmt scbs = currentBlockStmt;
        currentBlockStmt = bs;
     ...
     */
    private void buildDofio(Expression exp, BlockStmt bs)
            throws InvalidException
    {
        BlockStmt scbs = currentBlockStmt;
        currentBlockStmt = bs;
     ...
     */
    private void buildDouio(Expression exp, BlockStmt bs)
            throws InvalidException
    {
        BlockStmt scbs = currentBlockStmt;
        currentBlockStmt = bs;
            ...
            while (cont) {
                ...
                switch (keyword) {
                    ...
                    case Keywords.INTERFACE:
                        stmt = nextInterfaceBlock();
                        break;
                    case Keywords.PUBLIC:
                        stmt = nextPublicStmt();
                        break;
                    case Keywords.PRIVATE:
                        stmt = nextPrivateStmt();
                        break;
                    case Keywords.ALLOCATABLE:
                        stmt = nextAllocatableStmt();
                        break;
                    case Keywords.COMMON:
                        stmt = nextCommonStmt();
                        break;
                    case Keywords.DATA:
                        stmt = nextDataStmt();
                    ...
                    case Keywords.DIMENSION:
                        stmt = nextDimensionStmt();
                        break;
                    case Keywords.EQUIVALENCE:
                        stmt = nextEquivalenceStmt();
                        break;
                    case Keywords.EXTERNAL:
                        stmt = nextExternalStmt();
                        break;
                    case Keywords.INTENT:
                        stmt = nextIntentStmt();
                        break;
                    case Keywords.INTRINSIC:
                        stmt = nextIntrinsicStmt();
                        break;
                    case Keywords.NAMELIST:
                        stmt = nextNamelistStmt();
                        break;
                    case Keywords.OPTIONAL:
                        stmt = nextOptionalStmt();
                        break;
                    case Keywords.POINTER:
                        stmt = nextPointerStmt();
                        break;
                    case Keywords.SAVE:
                        stmt = nextSaveStmt();
                        break;
                    case Keywords.TARGET:
                        stmt = nextTargetStmt();
                        break;
                    default:
                        column = sc;
    ...
    private void nextEndStmt(int endType)
    {
        ...
        try {
            ...
            if (keyword == Keywords.END) {
                ...
                switch (keyword) {
                    case Keywords.PROGRAM:
                        keyword = Keywords.ENDPROGRAM;
                        break;
                    case Keywords.FUNCTION:
                        keyword = Keywords.ENDFUNCTION;
                        break;
                    case Keywords.BLOCKDATA:
                        keyword = Keywords.ENDBLOCKDATA;
                        break;
                    case Keywords.INTERFACE:
                        keyword = Keywords.ENDINTERFACE;
                        break;
                    case Keywords.SUBROUTINE:
                        keyword = Keywords.ENDSUBROUTINE;
                        break;
                    case Keywords.MODULE:
                        keyword = Keywords.ENDMODULE;
                        break;
                    case Keywords.TYPE:
                        keyword = Keywords.ENDTYPE;
                        break;
                    case Keywords.WHERE:
                        keyword = Keywords.ENDWHERE;
                        break;
                    case Keywords.SELECT:
                        keyword = Keywords.ENDSELECT;
                        break;
                    case Keywords.IF:
                        keyword = Keywords.ENDIF;
                        break;
                    case Keywords.FORALL:
                        keyword = Keywords.ENDFORALL;
                        break;
                    case Keywords.DO:
                        keyword = Keywords.ENDDO;
                        break;
                }
            }
            ...
            while (!eofStmt) {
                ...
                if ((name != null) && nextNBCharIs(':')) {
                    ...
                    if (Keywords.isConstruct(keyword)) {
                        ...
                        switch (keyword) {
                            case Keywords.DO:
                                stmt = nextDoStmt(name);
                                break;
                            case Keywords.FORALL:
                                stmt = nextForallStmt(name);
                                break;
                            case Keywords.IF:
                                stmt = nextIfStmt(name);
                                break;
                            case Keywords.SELECT:
                                stmt = nextSelectStmt(name);
                                break;
                            case Keywords.WHERE:
                                stmt = nextWhereStmt(name);
                                break;
                        }
                    }
                ...
                switch (keyword) {
                    ...
                    case Keywords.CASE:
                        ...
                        break;
                    case Keywords.DO:
                        stmt = nextDoStmt(null);
                        break;
                    case Keywords.FORALL:
                        stmt = nextForallStmt(null);
                        break;
                    case Keywords.IF:
                        stmt = nextIfStmt(null);
                        break;
                    case Keywords.WHERE:
                        stmt = nextWhereStmt(null);
                        break;
                    case Keywords.ALLOCATE:
                        stmt = nextAllocateStmt();
                        break;
                    case Keywords.BACKSPACE:
                        stmt = nextBackspaceStmt();
                        break;
                    case Keywords.CALL:
                        stmt = nextCallStmt();
                        break;
                    ...
                    case Keywords.CONTINUE:
                        stmt = nextContinueStmt();
                        break;
                    case Keywords.CYCLE:
                        stmt = nextCycleStmt();
                        break;
                    case Keywords.DEALLOCATE:
                        stmt = nextDeallocateStmt();
                        break;
                    case Keywords.ENDFILE:
                        stmt = nextEndfileStmt();
                        break;
                    case Keywords.EXIT:
                        stmt = nextExitStmt();
                        break;
                    case Keywords.GOTO:
                        stmt = nextGotoStmt();
                        break;
                    case Keywords.INQUIRE:
                        stmt = nextInquireStmt();
                        break;
                    case Keywords.NULLIFY:
                        stmt = nextNullifyStmt();
                        break;
                    case Keywords.OPEN:
                        stmt = nextOpenStmt();
                        break;
    ...
    private Statement nextActionStmt()
    {
        ...
        try {
            ...
            switch (keyword) {
                ...
                case Keywords.DO:
                    return nextDoStmt(null);
                case Keywords.ENDFILE:
                    return nextEndfileStmt();
                case Keywords.EXIT:
                    return nextExitStmt();
                case Keywords.FORALL:
                    return nextForallStmt(null);
                case Keywords.GOTO:
                    return nextGotoStmt();
                case Keywords.INQUIRE:
    ...
    private ArrayType nextArraySpec(Type elementType)
            throws InvalidException
    {
...
//             ((FormalDecl) vd).setMode(ParameterMode.VALUE);
    ...
    private Type nextKindSelector(int keyword)
            throws InvalidException
    {
    ...
    private Type nextCharLenParamValue()
            throws InvalidException
    {
    ...
    private Type nextCharKindSelector()
            throws InvalidException
    {
    ...
    private Type nextIntKindSelector()
            throws InvalidException
    {
    ...
    private Type nextLogicalKindSelector()
            throws InvalidException
    {
        ...
        try {
            ...
            if (x < 0) {
                switch (-x) {
                    case 1:
                        type = logicalTypeArray[1];
                        return type;
                    case 2:
                        type = logicalTypeArray[2];
                        return type;
                    case 4:
                        type = logicalTypeArray[3];
                        return type;
                    case 8:
                        type = logicalTypeArray[4];
                        return type;
                }
                userError(Msg.MSG_Invalid_KIND_selector, null);
            }
    ...
    private Type nextRealKindSelector()
            throws InvalidException
    {
    ...
    private Type nextComplexKindSelector()
            throws InvalidException
    {
    ...
    private void nextLetterSpecList(Type type)
            throws InvalidException
    {
    ...
    private ImpliedDo nextImpliedDo()
            throws InvalidException
    {
     ...
     */
    private long nextEqOffset(VariableDecl mv)
            throws InvalidException
    {
